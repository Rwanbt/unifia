...
function classify(filename) {
  ...
  if (/workbench-mode|design-surface|automate-surface|work-surface/i.test(filename)) return "mode"
...
for (const entry of readdirSync(ASSETS, { withFileTypes: true })) {
  ...
  prev.count += 1
  prev.bytes += bytes
  if (prev.files.length < 25) prev.files.push({ name, bytes })
...
const manifest = {
  ...
  byKind: Object.fromEntries(
    [...byKind.entries()].sort((a, b) => b[1].bytes - a[1].bytes).map(([k, v]) => [k, { count: v.count, bytes: v.bytes, files: v.files }]),
  ),
  // Budget per kind (H10 oracle). The Work / Design / Automate
  // / locales / terminal chunks must each stay under the per-surface
  // budget — exceeding it means a surface is pulling in too much
...
for (const [kind, data] of Object.entries(manifest.byKind)) {
  ...
  const status = budget && data.bytes > budget ? "OVER BUDGET" : "ok"
  console.log(`  ${kind.padEnd(16)} ${(data.bytes / 1024).toFixed(1).padStart(8)} KB  ${data.count.toString().padStart(4)} files  [${status}]`)
}
