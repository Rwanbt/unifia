...
- `Unifia` (preserve casing in prose, docs, and UI copy)
- Keep lowercase `unifia` in commands, package names, paths, URLs, and other exact identifiers
...
- Keep product naming consistent: `Unifia` in prose, `unifia` only for exact technical identifiers
...
- Avoid changing `unifia` to `Unifia` inside commands, URLs, package names, or code literals
