...
- Desktop deploy: copy **four** things from `packages/desktop/src-tauri/target/release/` to `C:/Users/barat/AppData/Local/Unifia Dev/`:
  1. `Unifia.exe`
  ...
  3. `livekit-server.exe` — the Live voice SFU sidecar (built from the pinned Go module by `scripts/livekit-server.ts`, SHA-256 pinned in `packages/voice-host/livekit-server.json`). It is verified against the bundled resource `voice-host/livekit-server.json` (copy it with the other `voice-host/` resources); without either, the Live button reports "Voice Host unavailable", and a binary whose hash differs is refused.
  4. `templates/design/` — the Design skill templates, resolved through `BaseDirectory::Resource` (i.e. next to the exe). Without it the composer's skill picker is empty, with no error.
...
- LiveKit sidecar (required before desktop build, needs Go; the script picks the pinned toolchain): `cd packages/desktop && bun scripts/livekit-server.ts x86_64-pc-windows-msvc` → `src-tauri/sidecars/livekit-server-x86_64-pc-windows-msvc.exe`. `predev.ts`/`prepare.ts` run it automatically.
