...
const { client, server } = createUnifia()
...
function createUnifia() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`unifia`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createUnifiaClient({ baseUrl: url })
...
async function assertUnifiaConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to unifia server")
  }
}
...
async function getAccessToken() {
  ...
  if (isMock()) {
    ...
    const oidcToken = await core.getIDToken("unifia-github-action")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "unifia-agent[bot]"`
  await $`git config --global user.email "unifia-agent[bot]@users.noreply.github.com"`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const shareUrl = shareId ? `[unifia session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
