...
- Fichier : `packages/unifia/src/observability/crash-reporter.ts`
...
- Fichier : `packages/unifia/src/server/routes/gdpr.ts`, branché dans `server/instance.ts`.
...
### I4 — Audit log (FAIT)
- Migration : `packages/unifia/migration/20260418120000_audit_log/migration.sql`.
- Table Drizzle : `packages/unifia/src/session/audit.sql.ts` (exportée via `storage/schema.ts`).
- Helper : `packages/unifia/src/session/audit.ts` — `AuditLog.record({ action, target, actor?, metadata?, force? })`, `AuditLog.list({ from, to, action, actor, limit })`.
...
- Fichier : `packages/unifia/src/security/scanner.ts`.
...
- Tests : `packages/unifia/test/security/scanner.test.ts` — 12 nouveaux cas (tous les nouveaux tokens + f-string + Ruby + prompt-injection).
...
- Fichier : `packages/unifia/src/provider/fallback.ts` — `withFallback(primary, secondary, {label?, shouldFallback?})`, `isNetworkRetryable()`.
...
- Fichier : `packages/unifia/test/e2e/dag-team.test.ts`.
...
## Fichiers créés
- `packages/unifia/src/observability/crash-reporter.ts`
- `packages/unifia/src/server/routes/gdpr.ts`
- `packages/unifia/src/session/audit.sql.ts`
- `packages/unifia/src/session/audit.ts`
- `packages/unifia/src/provider/fallback.ts`
- `packages/unifia/migration/20260418120000_audit_log/migration.sql`
- `packages/unifia/test/e2e/dag-team.test.ts`
- `SPRINT3_NOTES.md`

## Fichiers modifiés
- `packages/unifia/src/index.ts` (CrashReporter.init)
- `packages/unifia/src/config/config.ts` (experimental additions)
- `packages/unifia/src/security/scanner.ts` (+tokens, multiline, scanToolOutput)
- `packages/unifia/src/storage/schema.ts` (export AuditLogTable)
- `packages/unifia/src/server/instance.ts` (route GdprRoutes)
- `packages/unifia/src/local-llm-server/auto-config.ts` (startThermalListener)
- `packages/mobile/src-tauri/src/lib.rs` (get_thermal_state cmd stub)
- `packages/unifia/test/security/scanner.test.ts` (+12 tests)
