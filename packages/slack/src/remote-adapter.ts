...
import { ApprovalBroker, RemoteBridgeBroker, type RemoteCommand, type RemoteCommandResult, type RemoteMessage, type RemoteApprovalPort, type RemoteBridgePolicy, type RemoteVerifier } from "@unifia/contracts"
...
export class SlackRemoteAdapter {
  ...
  constructor(policy: RemoteBridgePolicy, verifier: RemoteVerifier, audit: { record(event: { type: "pair" | "revoke" | "message" | "replay" | "deny" | "approval"; identityId: string; reason?: string }): void }, now: () => number = () => Date.now(), approvals?: RemoteApprovalPort) {
    this.#now = now
