...
Replace runtime Bun shell template-tag usage in `packages/unifia/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/unifia/src` is removed except:
