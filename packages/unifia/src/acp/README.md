...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for unifia.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal unifia sessions
...
unifia acp --cwd /path/to/project
```
...
```bash
UNIFIA_ENABLE_QUESTION_TOOL=1 unifia acp
```
...
- **Terminal Integration**: Full terminal support via unifia's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | unifia acp
```
...
ACP sessions map cleanly to unifia's internal session model:
