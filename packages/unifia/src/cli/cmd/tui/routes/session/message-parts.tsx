...
import type { AssistantMessage, Part, ToolPart, UserMessage, TextPart, ReasoningPart } from "@unifia/sdk-shared"
...
import type { DebateLive, DebateTool } from "@/tool/debate"
  ...
  return (
    <Show when={props.part.text.trim()}>
      <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexShrink={0}>
        <Switch>
          <Match when={Flag.UNIFIA_EXPERIMENTAL_MARKDOWN}>
            <markdown
              syntaxStyle={syntax()}
              streaming={true}
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "debate"}>
          <Debate {...toolprops} />
        </Match>
        <Match when={true}>
...
function Debate(props: ToolProps<typeof DebateTool>) {
  ...
  const title = createMemo(() => {
    ...
    if (isRunning()) {
      const tier = props.input.tier ?? "auto"
      return `Debate (${tier}) — running…`
    }
    const blindSpots = meta?.blindSpotCount ?? 0
    const consensus = meta?.consensusCount ?? 0
    const providers = meta?.providerCount ?? 0
    const cost = meta?.cost != null ? `$${Number(meta.cost).toFixed(4)}` : ""
    return `Debate: ${blindSpots} blind spots, ${consensus} consensus (${providers} models) ${cost}`
  })
    ...
    >
      ...
      <Show when={!isRunning() && output()}>
        <box gap={1}>
          ...
          <Show when={overflow()}>
            <text fg={theme.textMuted}>{expanded() ? "Click to collapse" : "Click to expand full report"}</text>
          </Show>
        </box>
      </Show>
    </BlockTool>
