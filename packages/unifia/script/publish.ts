...
if (!process.env["UNIFIA_ALLOW_UPSTREAM_PUBLISH"]) {
  throw new Error(
    "packages/unifia/script/publish.ts targets upstream's registries (ghcr.io/anomalyco, AUR, anomalyco/homebrew-tap). " +
...
if (!Script.preview) {
  ...
  for (const [pkg, pkgbuild] of [["unifia-bin", binaryPkgbuild]]) {
    for (let i = 0; i < 30; i++) {
      try {
  ...
  await $`cd ./dist/homebrew-tap && git add unifia.rb`
