...
const server = createServer((_request, response) => { response.writeHead(200, { "content-type": "text/html" }); response.end("<main><h1>Unifia E2E</h1><p>local-only</p></main>") })
await new Promise<void>((resolve) => server.listen(0, "127.0.0.1", resolve))
const address = server.address()
assert.ok(address && typeof address === "object")
const root = await mkdtemp(join(tmpdir(), "unifia-browser-"))
const driver = new PlaywrightBrowserDriver(root)
const profile = { workspaceId: "workspace-e2e", profileId: "browser-workspace-e2e", hostAllowlist: ["127.0.0.1"], cookiesIsolated: true as const }
try {
