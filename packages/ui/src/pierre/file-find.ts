...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("unifia-find")
  api.delete("unifia-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("unifia-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("unifia-find", new Highlight(...rest))
    return true
  }
