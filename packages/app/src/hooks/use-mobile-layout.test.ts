...
describe("resolveMobileLayout", () => {
  test("uses the v110 overlay contract instead of legacy 768px buckets", () => {
    expect(resolveMobileLayout(701, 900, false)).toMatchObject({ isMobile: true, isTablet: true })
    expect(resolveMobileLayout(700, 900, false)).toMatchObject({ isMobile: true, isTablet: false })
