---
title: Plugins
description: Write your own plugins to extend Unifia.
---

Plugins allow you to extend Unifia by hooking into various events and customizing behavior. You can create plugins to add new features, integrate with external services, or modify Unifia's default behavior.
...
Unifia runs `bun install` at startup to install these. Your plugins and tools can then import them.
...
- `client`: An unifia SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend unifia.
...
:::note
If you’re using the Unifia desktop app, it can send system notifications automatically when a response is ready or when a session errors.
:::
...
Prevent unifia from reading `.env` files:
...
Plugins can also add custom tools to unifia:
...
The `tool` helper creates a custom tool that unifia can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to unifia alongside built-in tools.
