...
Unifia gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Unifia starter TUI for gjeldende katalog.
...
Når du bruker Unifia TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Unifia. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Unifia. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `unifia.json`, som konfigurerer server-/kjøretidsoppførsel.
...
Bruk `UNIFIA_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
