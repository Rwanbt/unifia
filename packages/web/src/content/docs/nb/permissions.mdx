...
Unifia bruker `permission`-konfigurasjonen for å bestemme om en gitt handling skal kjøres automatisk, spørre deg eller blokkeres.
...
Bruk `external_directory` for å tillate verktøyanrop som berører stier utenfor arbeidskatalogen der Unifia ble startet. Dette gjelder alle verktøy som tar en bane som input (for eksempel `read`, `edit`, `list`, `glob`, `grep` og mange `bash`-kommandoer).
...
Hvis du ikke spesifiserer noe, starter Unifia fra tillatte standardinnstillinger:
...
Når Unifia ber om godkjenning, gir brukergrensesnittet tre alternativer:
