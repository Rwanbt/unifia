...
Unifia integreres med VS Code, Cursor eller en hvilken som helst IDE som støtter en terminal. Bare kjør `unifia` i terminalen for å komme i gang.

---

## Bruk

- **Hurtigstart**: Bruk `Cmd+Esc` (Mac) eller `Ctrl+Esc` (Windows/Linux) for å åpne Unifia i en delt terminalvisning, eller fokuser en eksisterende terminaløkt hvis en allerede kjører.
...
- **Kontekstbevissthet**: Del automatisk ditt nåværende valg eller fane med Unifia.
...
For å installere Unifia på VS Code og populære varianter som Cursor, Windsurf, VSCodium:

1. Åpne VS Code
2. Åpne den integrerte terminalen
3. Kjør `unifia` - utvidelsen installeres automatisk
...
Søk etter **Unifia** i Extension Marketplace og klikk på **Install**.
...
- Sørg for at du kjører `unifia` i den integrerte terminalen.
