...
Du kan legge til eksterne verktøy i Unifia ved å bruke _Model Context Protocol_ eller MCP. Unifia støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Unifia Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
Unifia håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Unifia:
...
Hvis serveren krever autentisering, vil Unifia be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `unifia mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Unifia lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Unifia, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Unifia til Sentry-kontoen din.
