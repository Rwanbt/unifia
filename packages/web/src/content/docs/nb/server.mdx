...
Kommandoen `unifia serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `unifia.local` |
...
Sett `UNIFIA_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `unifia` som standard, eller sett `UNIFIA_SERVER_USERNAME` for å overstyre det. Dette gjelder både `unifia serve` og `unifia web`.
...
Når du kjører `unifia` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenCode-serveren til å samhandle med Unifia programmatisk.
:::

Denne arkitekturen lar Unifia støtte flere klienter og lar deg samhandle med Unifia programmatisk.

Du kan kjøre `unifia serve` for å starte en frittstående server. Hvis du har
Unifia TUI kjører, vil `unifia serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Unifia [IDE](/docs/ide) plugins.
