---
title: Konfigurasjon
description: Ved å bruke unifia JSON-konfigurasjonen.
---

Du kan konfigurere Unifia ved å bruke en JSON konfigurasjonsfil.

---

## Format

Unifia støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "unifia"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
3. **Egendefinert konfigurasjon** (`UNIFIA_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`unifia.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.opencode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`UNIFIA_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
Legg til `unifia.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Unifia starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `UNIFIA_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `UNIFIA_CONFIG_DIR`
...
Konfigurasjonsfilen har et skjema som er definert i [**`unifia.ai/config.json`**](https://opencode.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `unifia serve` og `unifia web` gjennom alternativet `server`.
...
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `unifia.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Unifia å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Unifia falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`unifia run`), skrivebordsapp og GitHub Action.
...
Unifia vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Unifia alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Unifia med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense Unifia til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
