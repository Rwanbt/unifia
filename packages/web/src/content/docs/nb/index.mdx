---
title: Introduksjon
description: Kom i gang med Unifia.
---
...
[**Unifia**](/) er en åpen kildekode AI kodeagent. Den er tilgjengelig som et terminalbasert grensesnitt, skrivebordsapp eller IDE-utvidelse.

![Unifia TUI med opencode-temaet](../../../assets/lander/screenshot.png)
...
For å bruke Unifia i terminalen din trenger du:
...
Den enkleste måten å installere Unifia på er gjennom installasjonsskriptet.
...
- **Bruke Homebrew på macOS og Linux**
  ...
  > Vi anbefaler å bruke OpenCode-tappen for de mest oppdaterte utgivelsene. Den offisielle `brew install unifia`-formelen vedlikeholdes av Homebrew-teamet og oppdateres sjeldnere.
...
For den beste opplevelsen på Windows anbefaler vi å bruke [Windows Subsystem for Linux (WSL)](/docs/windows-wsl). Det gir bedre ytelse og full kompatibilitet med funksjonene i Unifia.
:::
...
Støtte for installasjon av Unifia på Windows med Bun er under arbeid.
...
Med Unifia kan du bruke hvilken som helst LLM-leverandør ved å konfigurere deres API-nøkler.

Hvis du er ukjent med LLM-leverandører, anbefaler vi å bruke [Unifia Zen](/docs/zen).
Det er en kuratert liste over modeller som har blitt testet og verifisert av OpenCode-teamet.

1. Kjør kommandoen `/connect` i TUI, velg Unifia og gå til [unifia.ai/auth](https://opencode.ai/auth).
...
Deretter initialiserer du Unifia for prosjektet ved å kjøre følgende kommando.

```bash frame="none"
/init
```

Dette vil få Unifia til å analysere prosjektet ditt og opprette en `AGENTS.md`-fil i
prosjektroten.
...
Dette hjelper Unifia med å forstå prosjektstrukturen og kodingsmønstrene
brukt.
...
Du er nå klar til å bruke Unifia for å jobbe med prosjektet ditt. Spør det gjerne
noe!
...
Du kan be Unifia forklare kodebasen for deg.
...
Du kan be Unifia legge til nye funksjoner i prosjektet ditt. Selv om vi først anbefaler å be den om å lage en plan.

1. **Lag en plan**

   Unifia har en _Plan-modus_ som deaktiverer muligheten til å gjøre endringer og
   ...
   Du bør gi Unifia nok detaljer til å forstå hva du ønsker. Det hjelper
   å snakke med det som om du snakker med en juniorutvikler på laget ditt.

   :::tip
   Gi Unifia mye kontekst og eksempler for å hjelpe den å forstå hva du
   ønsker.
   :::
...
2. **Iterer på planen**
   ...
   Unifia kan skanne alle bilder du gir den og legge dem til i ledeteksten. Du kan
...
For mer enkle endringer kan du be Unifia om å bygge det direkte
...
Sørg for å gi nok detaljer, slik at Unifia gjør de rette
endringene.
...
La oss si at du ber Unifia om å gjøre noen endringer.
...
Unifia vil nå tilbakestille endringene du har gjort og vise den opprinnelige meldingen
igjen.
...
Herfra kan du justere ledeteksten og be Unifia om å prøve igjen.
...
Samtalene du har med Unifia kan [deles med din
team](/docs/share).
...
Her er en [eksempelsamtale](https://opencode.ai/s/4XP1fce5) med Unifia.

---

## Tilpasning

Og det er det! Du er nå en proff på å bruke Unifia.
