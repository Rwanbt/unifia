...
description: Skriv dine egne programtillegg for å utvide Unifia.
---

Plugins lar deg utvide Unifia ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Unifia.
...
2. Prosjektkonfigurasjon (`unifia.json`)
...
Unifia kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Unifia SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Unifia.
...
Hindre Unifia fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Unifia:
...
`tool`-hjelperen lager et tilpasset verktøy som Unifia kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Unifia sammen med innebygde verktøy.
