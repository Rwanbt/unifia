---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

Unifia bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Unifia
konfig.
...
Unifia Zen er en liste over modeller levert av OpenCode-teamet som har vært
testet og verifisert for å fungere godt med Unifia. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Unifia Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Unifia Zen` og gå til [unifia.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i Unifia og er helt valgfritt å bruke.

---

## Unifia Go

Unifia Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenCode-teamet som har vært
testet og verifisert for å fungere godt med Unifia.

1. Kjør kommandoen `/connect` i TUI, velg `Unifia Go`, og gå til [unifia.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Unifia:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Unifia:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `unifia.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i unifia støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Unifia skal fungere skikkelig.
   :::
...
for å lagre token i Unifia auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Unifia bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Unifia
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`unifia.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Unifia:
...
Slik bruker du Google Vertex AI med Unifia:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Unifia.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Unifia, kan du alltid konfigurere den selv.
...
Du kan konfigurere Unifia for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Unifia for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Unifia for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Unifia. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Unifia:
...
3. Kopier API-nøkkelen for bruk i Unifia.
...
6. **Viktig**: Før du bruker skymodeller i Unifia, må du hente modellinformasjonen lokalt:
...
Unifia Zen er en liste over testede og verifiserte modeller levert av OpenCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Unifia Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Unifia Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Unifia API-nøkkelen.
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Unifia:
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Unifia. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `unifia.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Unifia forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `unifia auth list` for å se om legitimasjonen
