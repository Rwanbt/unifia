---
title: Serwer
description: Interakcja z serwerem unifia za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `unifia serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient unifia.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `unifia.local` |
...
Ustaw `UNIFIA_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `unifia` lub ustaw `UNIFIA_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `unifia serve`, jak i `unifia web`.
...
Kiedy uruchomisz `unifia`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera unifia do programowej interakcji z unifia.
:::

Ta architektura umożliwia obsługę wielu klientów przez unifia i programową interakcję z kodem otwartym.

Możesz uruchomić `unifia serve`, aby uruchomić samodzielny serwer. Jeśli masz
unifia TUI działa, `unifia serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki unifia [IDE](/docs/ide).
...
Serwer unifia udostępnia następujące interfejsy API.
