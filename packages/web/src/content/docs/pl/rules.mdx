---
title: Zasady
description: Ustaw niestandardowe instrukcje dla unifia.
---

Możesz dostarczyć niestandardowe instrukcje do unifia, tworząc plik `AGENTS.md`. Jest to podobne do zasad Cursora. Zawiera instrukcje, które zostaną uwzględnione w kontekście LLM, aby dostosować jego zachowanie do konkretnego projektu.
...
unifia obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/opencode/AGENTS.md`. Jest to stosowane we wszystkich sesjach unifia.
...
W przypadku użytkowników migrujących z Claude Code unifia obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:
...
Po uruchomieniu unifia szuka plików reguł w następującej kolejności:
...
Możesz określić niestandardowe pliki instrukcji w swoim `unifia.json` lub globalnym `~/.config/opencode/opencode.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Chociaż unifia nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:

### Używanie unifia.json

Zalecanym podejściem jest użycie pola `instructions` w `unifia.json`:
...
Możesz nauczyć unifia czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że unifia ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania

:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `unifia.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
