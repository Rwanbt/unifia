...
Umiejętności agenta pozwalają unifia odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
unifia przeszukuje te lokalizacje:
...
W przypadku ścieżek lokalnych projektu unifia przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
...
unifia wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `unifia.json`:
...
**Dla agentów wbudowanych** (w `unifia.json`):
