...
Możesz dodać dodatkowe informacje do unifia za pomocą _Model Context Protocol_, w skrócie MCP. unifia obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [unifia Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
unifia automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, unifia:
...
Jeśli serwer wymaga uwierzytelnienia, unifia poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `unifia mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji unifia bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w unifia, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie unifia, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć unifia z kontem Sentry.
