---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

unifia używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji unifia.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim unifia
config.
...
Unifia Zen to lista modeli dostarczonych przez zespół unifia, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z unifia. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Unifia Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Unifia Zen` i przejdź do [unifia.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w unifia i jest całkowicie opcjonalny w użyciu.

---

## Unifia Go

Unifia Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół unifia, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z unifia.

1. Uruchom polecenie `/connect` w TUI, wybierz `Unifia Go` i przejdź do [unifia.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z unifia:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania unifia:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `unifia.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w unifia nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod unifia działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji unifia.
...
unifia używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować unifia
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`unifia.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem unifia:
...
Aby używać Google Vertex AI z unifia:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania unifia.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą unifia, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację unifia jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja unifia będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować unifia tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować unifia tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować unifia tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem unifia. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z unifia:
...
3. Skopiuj klucz API do użycia w unifia.
...
6. **Ważne**: Przed użyciem modeli chmurowych w unifia musisz pobrać informacje o modelu lokalnie:
...
Unifia Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół unifia. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Unifia Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Unifia Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API unifia.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji unifia.
...
5. Możesz je także dostosować za pomocą konfiguracji unifia. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z unifia:
...
Możesz także dostosować modele za pomocą konfiguracji unifia. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z unifia. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `unifia.json` w katalogu projektu:
...
Pola `limit` pozwalają unifia zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `unifia auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację unifia i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji unifia.
