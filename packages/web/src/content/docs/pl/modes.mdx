...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji unifia.
...
te lub skonfiguruj własne za pomocą konfiguracji unifia.
...
Skonfiguruj tryb w pliku konfiguracyjnym `unifia.json`:
...
Jeśli nie ma wpływu na temperaturę, unifia stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna unifia, jak i specjalna dla projektu.
