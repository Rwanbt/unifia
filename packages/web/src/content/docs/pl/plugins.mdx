---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć unifia.
---

Wtyczki umożliwiające wyłączenie unifia poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania unifia.
...
2. Konfiguracja projektu (`unifia.json`)
...
unifia uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z unifia do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do unifia.
...
:::note
Jeśli korzystasz z aplikacji komputerowej unifia, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj unifia czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do unifia:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod unifia. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
