---
title: Konfiguracja
description: Ustawienia Unifia w pliku JSON.
---

Możesz dostosować Unifia za pomocą pliku konfiguracyjnego JSON.

---

## Format

Unifia obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "unifia"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
3. **Konfiguracja niestandardowa** (zmienna `UNIFIA_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`unifia.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.opencode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `UNIFIA_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
Umieść swoją globalną konfigurację Unifia w `~/.config/opencode/opencode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.
...
Dodaj `unifia.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Unifia się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `UNIFIA_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `UNIFIA_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.opencode` i powinien zachować tę samą strukturę.
...
Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`unifia.ai/config.json`**](https://opencode.ai/config.json).
...
Użyj `UNIFIA_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `unifia.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `unifia serve` i `unifia web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Unifia.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `unifia.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Unifia wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Unifia powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`unifia run`), aplikacji desktopowej i GitHub Actions.
...
Unifia może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Unifia nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Unifia **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Unifia o niestandardowe narzędzia, hooki i integracje.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Unifia tylko do określonych dostawców.
