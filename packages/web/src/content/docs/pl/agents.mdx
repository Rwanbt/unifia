...
W Unifia obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Unifia zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Unifia ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Unifia ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `unifia.json`:
...
Jeśli nie zostanie skonfigurowana, Unifia używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Unifia, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Unifia w `provider/model-id`. Na przykład, [Unifia Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `unifia models`, aby zobaczyć dostępne modele.
:::
