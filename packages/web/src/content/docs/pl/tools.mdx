...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Unifia posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Unifia.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `UNIFIA_EXPERIMENTAL_LSP_TOOL=true` (lub `UNIFIA_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Unifia lub gdy zmienna środowiskowa `UNIFIA_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Unifia:
