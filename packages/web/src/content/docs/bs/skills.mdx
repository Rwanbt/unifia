...
Agent skills omogucavaju Unifia da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Unifia pretrazuje ove lokacije:
...
Za projektne lokalne putanje, Unifia ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
...
Unifia navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `unifia.json`:
...
**Za ugradene agente** (u `unifia.json`):
