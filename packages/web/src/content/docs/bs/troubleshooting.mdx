...
Da biste otklonili probleme s Unifia, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\unifia\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `unifia --log-level DEBUG`.

---

## Pohrana

Unifia pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\unifia`
...
Unifia Desktop pokreće lokalni Unifia server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Unifia` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\unifia\unifia.jsonc`
...
Unifia također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\unifia\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Unifia mogao ponovo izgraditi.

1. Potpuno zatvorite Unifia Desktop.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\unifia`

3. Ponovo pokrenite Unifia Desktop.

---

### Rješavanje problema sa vezom na serveru

Unifia Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `unifia.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `UNIFIA_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `UNIFIA_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Unifia Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Unifia karakteristikama.

---

### Obavještenja se ne prikazuju

Unifia Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Unifia u postavkama vašeg OS-a, i
...
1. Zatvorite Unifia Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Unifia Desktop aplikacije):

- `unifia.settings.dat` (URL zadanog servera za desktop)
- `unifia.global.dat` i `unifia.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `unifia models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\unifia`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Unifia dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\unifia`

2. Ponovo pokrenite Unifia da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Unifia da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Unifia će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
