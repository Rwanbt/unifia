---
title: Pravila
description: Postavite prilagodena uputstva za unifia.
---

Mozete dodati prilagodena uputstva za unifia tako sto kreirate `AGENTS.md` datoteku. Ovo je slicno pravilima u Cursor-u. Sadrzi uputstva koja se ubacuju u LLM kontekst da prilagode ponasanje za vas projekat.

---

## Inicijalizacija

Da kreirate novu `AGENTS.md` datoteku, pokrenite `/init` komandu u unifia.

:::tip
Preporuceno je da `AGENTS.md` iz projekta commitujete u Git.
:::

Ovo skenira projekat i njegov sadrzaj, razumije cemu projekat sluzi i generise `AGENTS.md`. Tako unifia bolje navigira kroz kod.
...
unifia podrzava citanje `AGENTS.md` datoteke sa vise lokacija. Svaka lokacija ima drugu svrhu.
...
Mozete imati i globalna pravila u `~/.config/opencode/AGENTS.md`. Ona se primjenjuju u svim unifia sesijama.
...
Za korisnike koji prelaze sa Claude Code, Unifia podrzava i Claude konvencije datoteka kao rezervu:
...
Kada se unifia pokrene, trazi datoteke pravila ovim redoslijedom:
...
Mozete navesti prilagodene datoteke uputstava u `unifia.json` ili globalnom `~/.config/opencode/opencode.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
Iako unifia ne parsira automatski reference datoteka u `AGENTS.md`, slicno ponasanje mozete dobiti na dva nacina:

### Korištenje unifia.json

Preporuceni pristup je da koristite `instructions` polje u `unifia.json`:
...
Mozete nauciti unifia da cita eksterne datoteke tako sto cete dati eksplicitna uputstva u `AGENTS.md`. Evo prakticnog primjera:
...
- Osigurate da unifia ucitava datoteke samo kad su potrebne za konkretan zadatak

:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `unifia.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
