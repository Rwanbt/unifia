---
title: Konfiguracija
description: Korištenje Unifia JSON konfiguracije.
---

Možete konfigurirati Unifia koristeći JSON konfiguracijski fajl.

---

## Format

Unifia podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
3. **Prilagođena konfiguracija** (`UNIFIA_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`unifia.json` u projektu) - postavke specifične za projekat
5. **`.opencode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`UNIFIA_CONFIG_CONTENT` env var) - runtime preinačenja
...
Postavite svoju globalnu Unifia konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `unifia.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Unifia pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `UNIFIA_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `UNIFIA_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`unifia.ai/config.json`**](https://opencode.ai/config.json).

TUI konfiguracija koristi [**`unifia.ai/tui.json`**](https://opencode.ai/tui.json).
...
Koristite `UNIFIA_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `unifia.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `unifia serve` i `unifia web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Unifia server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `unifia.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Unifia konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Unifia pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Unifia će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`unifia run`), desktop aplikaciju i GitHub Action.
...
Unifia će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Unifia **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Unifia sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite Unifia da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
