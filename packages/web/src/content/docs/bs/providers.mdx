---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

Unifia koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Unifia konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Unifia
config.
...
Unifia Zen je lista modela koje je obezbedio Unifia tim koji su bili
testirano i potvrđeno da dobro radi sa Unifia. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Unifia Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite unifia i idite na [unifia.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Unifia i potpuno je opcionalan za korištenje.

---

## Unifia Go

Unifia Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Unifia tim i koji su testirani i verificirani da dobro rade s OpenCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Unifia Go` i idite na [unifia.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Unifia:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete unifia:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `unifia.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Unifia.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi unifia ispravno radio.
   :::
...
Također možete dodati modele kroz svoju unifia konfiguraciju.
...
da pohrani token u unifia auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Unifia koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Unifia
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`unifia.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s unifia:
...
Za korištenje Google Vertex AI s Unifia:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće unifia.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Unifia razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Unifia razgovor naveden kao zasebna sesija.
...
Možete konfigurirati unifia za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati unifia za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati unifia za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Unifia. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Unifia:
...
3. Kopirajte API ključ za korištenje u Unifia.
...
6. **Važno**: Prije upotrebe modela oblaka u Unifia, morate lokalno povući informacije o modelu:
...
Unifia Zen je lista testiranih i verifikovanih modela koju je obezbedio Unifia tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Unifia Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Unifia Zen**.

```txt
   /connect
```

3. Unesite svoj Unifia API ključ.
...
Također možete dodati dodatne modele putem vaše unifia konfiguracije.
...
5. Također ih možete prilagoditi putem vaše unifia konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Unifiaom:
...
Također možete prilagoditi modele kroz svoju unifia konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `unifia.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Unifia da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `unifia auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Unifia konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj unifia konfiguraciji.
