...
Unifia Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
Unifia Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u OpenCode-u. Pretplatite se na Unifia Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Unifia.
...
Unifia Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Unifia Go radi kao bilo koji drugi provajder u OpenCode-u.

1. Prijavite se na **<a href={console}>Unifia Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Unifia Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Unifia Go.
:::
...
Unifia Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Unifia konfiguraciji
...
Napravili smo Unifia Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Unifia koristite i bilo kojeg drugog provajdera.
