...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Unifia dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Unifia.
...
:::note
Ovaj alat je dostupan samo kada je `UNIFIA_EXPERIMENTAL_LSP_TOOL=true` (ili `UNIFIA_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Unifia provajdera ili kada je varijabla `UNIFIA_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Unifia:
