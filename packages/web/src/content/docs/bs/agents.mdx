...
Postoje dvije vrste agenata u Unifia: primarni agenti i podagenti.
...
Unifia dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Unifia dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Unifia dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `unifia.json`:
...
Ako temperatura nije navedena, Unifia koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Unifia konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Unifia konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Unifia Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `unifia models` da vidite listu dostupnih modela.
:::
