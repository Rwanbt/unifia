---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Unifia.
---

Dodaci vam omogućavaju da proširite Unifia spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Unifia.
...
2. Konfiguracija projekta (`unifia.json`)
...
Unifia pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Unifia SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Unifia.
...
Spriječite unifia da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u unifia:
...
Pomoćnik `tool` kreira prilagođeni alat koji unifia može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za unifia zajedno sa ugrađenim alatima.
