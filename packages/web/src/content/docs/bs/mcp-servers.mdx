...
Možete dodati vanjske alate u Unifia koristeći _Model Context Protocol_, ili MCP. Unifia podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Unifia Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Unifia automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Unifia će:
...
Ako server zahtijeva autentifikaciju, Unifia će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `unifia mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Unifia sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Unifia, zajedno s ugrađenim alatima. Možete njima upravljati kroz Unifia konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Unifia sa vašim Sentry nalogom.
