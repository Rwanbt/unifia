---
title: Server
description: Komunicirajte s unifia serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `unifia serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji unifia klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `unifia.local` |
...
Postavite `UNIFIA_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `unifia`, ili postavite `UNIFIA_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `unifia serve` i za `unifia web`.
...
Kada pokrenete `unifia`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite unifia server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava unifia podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `unifia serve` da startate standalone server. Ako je unifia TUI vec pokrenut, `unifia serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Unifia [IDE](/docs/ide) pluginovi.
...
unifia server izlaže sljedece API-je.
