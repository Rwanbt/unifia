---
title: Providers
description: Using any LLM provider in Unifia.
---

import config from "../../../config.mjs"
export const console = config.console

Unifia uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Unifia config.
...
You can customize the providers through the `provider` section in your Unifia
config.
...
Unifia Zen is a list of models provided by the Unifia team that have been
tested and verified to work well with Unifia. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Unifia Zen.
:::

1. Run the `/connect` command in the TUI, select `Unifia Zen`, and head to [unifia.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Unifia and is completely optional to use.

---

## Unifia Go

Unifia Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Unifia team that have been
tested and verified to work well with Unifia.

1. Run the `/connect` command in the TUI, select `Unifia Go`, and head to [unifia.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Unifia:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running unifia:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `unifia.json`:
...
Unifia. Anthropic explicitly prohibits this.

Previous versions of Unifia came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Unifia with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for unifia to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your unifia config.
...
GitLab Duo support in Unifia is experimental. Features, configuration, and
behavior may change in future releases.
:::

Unifia integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
Unifia uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Unifia
to only use your own GitLab-hosted instance, add the following to your
`unifia.json` file. It is also recommended to disable session sharing.
...
When a `duo-workflow-*` model is selected, Unifia will:
...
4. Route tool execution requests through Unifia's permission-gated tool system
...
To use your GitHub Copilot subscription with unifia:
...
To use Google Vertex AI with Unifia:
...
In the event you see a feature or model from Helicone that isn't configured automatically through unifia, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Unifia conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Unifia conversation listed as a separate session.
...
You can configure unifia to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure unifia to use local models through LM Studio.
...
You can configure unifia to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Unifia. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with Unifia:
...
3. Copy the API key for use in Unifia.
...
6. **Important**: Before using cloud models in Unifia, you must pull the model information locally:
...
Unifia Zen is a list of tested and verified models provided by the Unifia team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Unifia Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Unifia Zen**.

   ```txt
   /connect
   ```

3. Enter your Unifia API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your unifia config.
...
5. You can also customize them through your unifia config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Unifia:
...
You can also customize models through your unifia config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with unifia. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `unifia.json` file in your project directory:
...
The `limit` fields allow Unifia to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `unifia auth list` to see if the credentials
...
2. For custom providers, check the unifia config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your unifia config.
