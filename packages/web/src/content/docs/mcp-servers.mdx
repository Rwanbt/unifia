...
You can add external tools to Unifia using the _Model Context Protocol_, or MCP. Unifia supports both local and remote servers.
...
You can define MCP servers in your [Unifia Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Unifia automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Unifia will:
...
If the server requires authentication, Unifia will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `unifia mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Unifia will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Unifia, alongside built-in tools. So you can manage them through the Unifia config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Unifia to your Sentry account.
