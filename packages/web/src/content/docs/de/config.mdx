---
title: Konfiguration
description: Verwenden Sie die Konfiguration Unifia JSON.
---

Sie können Unifia mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

Unifia unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Wenn Ihre globale Konfiguration beispielsweise `theme: "unifia"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
3. **Benutzerdefinierte Konfiguration** (`UNIFIA_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`unifia.json` im Projekt) – projektspezifische Einstellungen
5. **`.opencode` Verzeichnisse** – Agenten, Befehle, Plugins
6. **Inline-Konfiguration** (`UNIFIA_CONFIG_CONTENT` env var) – Laufzeitüberschreibungen
...
Fügen Sie `unifia.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.

:::tip
Platzieren Sie die projektspezifische Konfiguration im Stammverzeichnis Ihres Projekts.
:::

Wenn Unifia gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
Geben Sie mithilfe der Umgebungsvariablen `UNIFIA_CONFIG` einen benutzerdefinierten Konfigurationsdateipfad an.
...
Geben Sie mit `UNIFIA_CONFIG_DIR` ein benutzerdefiniertes Konfigurationsverzeichnis an.
...
Die Konfigurationsdatei verfügt über ein Schema, das in [**`unifia.ai/config.json`**](https://opencode.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `unifia serve` und `unifia web` über die Option `server` konfigurieren.
...
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `unifia.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht Unifia, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift Unifia mit einer Warnung auf `"build"` zurück.

Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`unifia run`), Desktop-App und GitHub Aktion.
...
Unifia lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
Unifia erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert Unifia mit benutzerdefinierten Tools, Hooks und Integrationen.
...
Dies ist nützlich, wenn Sie Unifia darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
