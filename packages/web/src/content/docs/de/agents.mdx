...
Es gibt zwei Arten von Agenten in Unifia; Primäragenten und Subagenten.
...
Unifia verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `unifia.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Unifia modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Die Modell-ID in Ihrer OpenCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Unifia Zen](/docs/zen) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `unifia models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
