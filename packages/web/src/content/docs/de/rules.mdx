---
title: Regeln
description: Lege eigene Anweisungen fuer unifia fest.
---

Du kannst unifia ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in unifia den Befehl `/init` aus.
...
So kann unifia sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht unifia sie zu erweitern.
...
unifia kann `AGENTS.md` aus mehreren Orten lesen.
...
Wenn du von Claude Code kommst, unterstuetzt Unifia dessen Dateikonventionen als Fallback:
...
Beim Start sucht unifia in dieser Reihenfolge nach Regeldateien:
...
Du kannst zusaetzliche Anweisungsdateien in deiner `unifia.json` oder in `~/.config/opencode/opencode.json` angeben.
...
unifia parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit unifia.json

Empfohlen ist das `instructions`-Feld in der `unifia.json`:
...
Du kannst unifia explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `unifia.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
