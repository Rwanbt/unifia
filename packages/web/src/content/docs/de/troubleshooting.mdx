...
Wenn Unifia Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\unifia\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `unifia --log-level DEBUG`.

---

## Speicher

unifia speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\unifia`
...
Unifia Desktop startet im Hintergrund einen lokalen OpenCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `Unifia`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\unifia\unifia.jsonc`
...
Unifia kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\unifia\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Unifia ihn neu aufbauen kann.

1. Beende Unifia Desktop komplett.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\unifia`

3. Starte Unifia Desktop neu.

---

### Server-Verbindungsprobleme beheben

Unifia Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `unifia.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `UNIFIA_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `UNIFIA_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Unifia Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.
...
Unifia Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Unifia in den OS-Einstellungen aktiviert sind, und
...
1. Beende Unifia Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Unifia Desktop):

- `unifia.settings.dat` (desktop default server URL)
- `unifia.global.dat` und `unifia.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Unifia hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `unifia models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\unifia`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. unifia installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\unifia`

2. Starte unifia neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt unifia, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
unifia erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
