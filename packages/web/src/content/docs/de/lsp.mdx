---
title: LSP Server
description: Unifia lässt sich in Ihren LSP-Server integrieren.
---

Unifia lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

Unifia verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `UNIFIA_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn Unifia eine Datei öffnet, geschieht Folgendes:
