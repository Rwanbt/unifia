---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

Unifia verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Unifia anpassen
config.
...
Unifia Zen ist eine Liste von Modellen, die vom OpenCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Unifia funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Unifia Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Unifia aus und gehen Sie zu [unifia.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Unifia und ist völlig optional.

---

## Unifia Go

Unifia Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenCode-Team getestet und verifiziert wurden, dass sie gut mit Unifia funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Unifia Go` und gehen Sie zu [unifia.ai/auth](https://opencode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Unifia:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Unifia ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `unifia.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Unifia wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Unifia ordnungsgemäß funktioniert.
   :::
...
Unifia verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Unifia
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`unifia.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Unifia:
...
So verwenden Sie Google Vertex AI mit Unifia:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Unifia ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Unifia konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Sie können Unifia für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Unifia über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Unifia über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Unifia konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit Unifia:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Unifia.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Unifia verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Unifia Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Unifia Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Unifia Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Unifia API-Schlüssel ein.
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Unifia:
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Unifia verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `unifia.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Unifia erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `unifia auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
