...
Unifia lässt sich in VS Code, Cursor oder jede IDE integrieren, die ein Terminal unterstützt. Führen Sie einfach `unifia` im Terminal aus, um zu beginnen.

---

## Verwendung

- **Schnellstart**: Verwenden Sie `Cmd+Esc` (Mac) oder `Ctrl+Esc` (Windows/Linux), um Unifia in einer geteilten Terminalansicht zu öffnen, oder fokussieren Sie eine dieser Terminalsitzung, falls bereits eine ausgeführt wird.
...
- **Kontextbewusstsein**: Teilen Sie Ihre aktuelle Auswahl oder Registerkarte automatisch mit Unifia.
...
So installieren Sie Unifia in VS Code und beliebten Forks wie Cursor, Windsurf, VSCodium:

1. Öffnen Sie VS Code
2. Öffnen Sie das integrierte Terminal
3. Führen Sie `unifia` aus – die Erweiterung wird automatisch installiert
...
Suchen Sie im Extension Marketplace nach **Unifia** und klicken Sie auf **Installieren**.
...
- Stellen Sie sicher, dass Sie `unifia` im integrierten Terminal ausführen.
