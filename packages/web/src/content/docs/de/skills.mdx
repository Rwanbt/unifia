...
Agent Skills erlauben Unifia, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Unifia sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Unifia vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
...
Unifia listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `unifia.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `unifia.json`):
