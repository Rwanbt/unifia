...
Der Befehl `unifia serve` startet einen headless HTTP-Server.
...
Setze `UNIFIA_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `unifia`, kann aber mit `UNIFIA_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `unifia serve` und `unifia web`.
...
Wenn du `unifia` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den opencode-Server, um unifia programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `unifia serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `unifia serve` trotzdem eine neue Serverinstanz.
