---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Unifia zu erweitern.
---

Mit Plugins können Sie Unifia erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Unifia zu ändern.
...
Unifia führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Unifia erweitern können.
...
Verhindern Sie, dass Unifia `.env`-Dateien liest:
...
Plugins können Unifia auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Unifia aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Unifia zur Verfügung.
