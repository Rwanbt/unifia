...
Sie können externe Tools zu Unifia hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Unifia unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Unifia Config](https://opencode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.

```jsonc title="unifia.jsonc" {6}
{
...
Unifia übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Unifia Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Unifia beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `unifia mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Unifia die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Unifia verfügbar. Sie können sie also wie jedes andere Tool über die OpenCode-Konfiguration verwalten.
...
```json title="unifia.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Unifia mit Ihrem Sentry-Konto zu verbinden.
