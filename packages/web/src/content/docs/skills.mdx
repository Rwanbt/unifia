...
Agent skills let Unifia discover reusable instructions from your repo or home directory.
...
Unifia searches these locations:
...
For project-local paths, Unifia walks up from your current working directory until it reaches the git worktree.
...
Unifia lists available skills in the `skill` tool description.
...
Control which skills agents can access using pattern-based permissions in `unifia.json`:
...
**For built-in agents** (in `unifia.json`):
