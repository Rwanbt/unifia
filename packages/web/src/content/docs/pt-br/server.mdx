---
title: Servidor
description: Interaja com o servidor unifia via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `unifia serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente unifia pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `unifia.local` |
...
Defina `UNIFIA_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `unifia`, ou defina `UNIFIA_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `unifia serve` quanto ao `unifia web`.

```bash
UNIFIA_SERVER_PASSWORD=your-password unifia serve
```
...
Quando você executa `unifia`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor unifia para interagir com o unifia programaticamente.
:::

Essa arquitetura permite que o unifia suporte múltiplos clientes e permite que você interaja com o unifia programaticamente.

Você pode executar `unifia serve` para iniciar um servidor autônomo. Se você tiver o TUI do unifia em execução, `unifia serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do unifia [IDE](/docs/ide).
...
O servidor unifia expõe as seguintes APIs.
