---
title: Provedores
description: Usando qualquer provedor LLM no unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

unifia usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do unifia.
...
Você pode personalizar os provedores através da seção `provider` na sua configuração do unifia.
...
Unifia Zen é uma lista de modelos fornecidos pela equipe do unifia que foram testados e verificados para funcionar bem com o unifia. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Unifia Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Unifia Zen` e acesse [unifia.ai/zen](https://opencode.ai/zen).
...
Funciona como qualquer outro provedor no unifia e é completamente opcional.

---

## Unifia Go

Unifia Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do unifia que foram
testados e verificados para funcionar bem com o unifia.

1. Execute o comando `/connect` no TUI, selecione `Unifia Go` e acesse [unifia.ai/zen](https://opencode.ai/zen).
...
Para usar o Amazon Bedrock com o unifia:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o unifia:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `unifia.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no unifia não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o unifia funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do unifia.
...
armazenar o token no armazenamento de autenticação do unifia.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
unifia usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o unifia
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `unifia.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `unifia.json`:
...
Para usar sua assinatura do GitHub Copilot com o unifia:
...
Para usar o Google Vertex AI com o unifia:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o unifia.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do unifia, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do unifia como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do unifia listada como uma sessão separada.
...
Você pode configurar o unifia para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o unifia para usar modelos locais através do LM Studio.
...
Você pode configurar o unifia para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o unifia. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/opencode) para detalhes.
:::
...
Para usar o Ollama Cloud com o unifia:
...
3. Copie a chave da API para uso no unifia.
...
6. **Importante**: Antes de usar modelos em nuvem no unifia, você deve puxar as informações do modelo localmente:
...
Unifia Zen é uma lista de modelos testados e verificados fornecidos pela equipe do unifia. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Unifia Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Unifia Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do unifia.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do unifia.
...
5. Você também pode personalizá-los através da sua configuração do unifia. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o unifia:
...
Você também pode personalizar modelos através da sua configuração do unifia. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o unifia. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no unifia.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `unifia.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o unifia entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `unifia auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do unifia e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do unifia.
