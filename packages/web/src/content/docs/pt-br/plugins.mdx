---
title: Plugins
description: Escreva seus próprios plugins para estender o unifia.
---

Plugins permitem que você estenda o unifia conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do unifia.
...
2. Configuração do projeto (`unifia.json`)
...
O unifia executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do unifia para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o unifia.
...
:::note
Se você estiver usando o aplicativo desktop unifia, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o unifia de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao unifia:
...
O helper `tool` cria uma ferramenta personalizada que o unifia pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o unifia junto com as ferramentas integradas.
