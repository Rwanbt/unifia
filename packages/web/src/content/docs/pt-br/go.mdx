...
O Unifia Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O Unifia Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no Unifia. Você assina o Unifia Go e
...
O Unifia Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Unifia Go funciona como qualquer outro provedor no Unifia.

1. Você entra no **<a href={console}>Unifia Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Unifia Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Unifia Go.
:::
...
O Unifia Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Unifia
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Unifia também.
