...
Existem dois tipos de agentes no unifia; agentes primários e subagentes.
...
unifia vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
unifia vem com dois subagentes integrados, **General** e **Explore**. Vamos ver isso abaixo.

---

## Integrados

unifia vem com dois agentes primários integrados e dois subagentes integrados.
...
Configure os agentes em seu arquivo de configuração `unifia.json`:
...
Se nenhuma temperatura for especificada, o unifia usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do unifia quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do unifia usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Unifia Zen](/docs/zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `unifia models` para ver uma lista dos modelos disponíveis.
:::
