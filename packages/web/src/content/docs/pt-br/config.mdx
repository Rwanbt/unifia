---
title: Configuração
description: Usando a configuração JSON do unifia.
---

Você pode configurar o unifia usando um arquivo de configuração JSON.

---

## Formato

O unifia suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
3. **Configuração personalizada** (`UNIFIA_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`unifia.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.opencode`** - agentes, comandos, plugins
6. **Configuração inline** (`UNIFIA_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
Coloque sua configuração global do unifia em `~/.config/opencode/opencode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `unifia.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o unifia é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `UNIFIA_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `UNIFIA_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`unifia.ai/config.json`**](https://opencode.ai/config.json).

A configuração do TUI usa [**`unifia.ai/tui.json`**](https://opencode.ai/tui.json).
...
Use `UNIFIA_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `unifia.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `unifia serve` e `unifia web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor unifia.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `unifia.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do unifia através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o unifia tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o unifia recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`unifia run`), aplicativo desktop e GitHub Action.
...
O unifia fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o unifia **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o unifia com ferramentas, hooks e integrações personalizadas.
...
Isso é útil quando você deseja restringir o unifia para usar apenas provedores específicos, em vez de desabilitá-los um a um.
