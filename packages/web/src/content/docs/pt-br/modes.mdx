...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do unifia. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no unifia permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do unifia.
...
O unifia vem com dois modos integrados.
...
Configure os modos no seu arquivo de configuração `unifia.json`:
...
Se nenhuma temperatura for especificada, o unifia usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do unifia quanto para a configuração específica do projeto.
