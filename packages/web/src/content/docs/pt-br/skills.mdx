...
As habilidades do agente permitem que o unifia descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O unifia pesquisa nesses locais:
...
Para caminhos locais do projeto, o unifia sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
...
O unifia lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `unifia.json`:
...
**Para agentes embutidos** (em `unifia.json`):
