...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. unifia yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
unifia içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `UNIFIA_EXPERIMENTAL_LSP_TOOL=true` (veya `UNIFIA_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca unifia provider kullanılırken veya `UNIFIA_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

unifia başlatırken etkinleştirmek için:
