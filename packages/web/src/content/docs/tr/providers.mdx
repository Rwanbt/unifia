---
title: Sağlayıcılar
description: unifia'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

unifia, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı unifia yapılandırmanızda yapılandırın.
...
Sağlayıcıları unifia'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Unifia Zen, unifia ekibi tarafından sağlanan modellerin bir listesidir.
unifia ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Unifia Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, unifia'u seçin ve [unifia.ai/auth](https://opencode.ai/auth)'ye gidin.
...
unifia'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Unifia Go

Unifia Go, unifia ile iyi çalıştığı test edilmiş ve doğrulanmış, unifia ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Unifia Go`'yu seçin ve [unifia.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u unifia ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   unifia'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `unifia.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi unifia'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   unifia'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Unifia yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in unifia auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
unifia, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. unifia'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`unifia.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`unifia.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi unifia'la kullanmak için:
...
Google Vertex AI'yi unifia ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları unifia çalıştırılırken ayarlayın.
...
Helicone'un unifia aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her unifia görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her unifia görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
unifia'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla unifia'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla unifia'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini unifia için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::
...
Ollama Cloud'u unifia ile kullanmak için:
...
3. unifia'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: unifia'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Unifia Zen, unifia ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Unifia Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Unifia Zen**'i arayın.

   ```txt
   /connect
   ```

3. unifia API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   unifia yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca unifia yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Unifia ile kullanmak için:
...
Modelleri unifia yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı unifia ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `unifia.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları unifia'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `unifia auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için unifia yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, unifia yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
