---
title: Sunucu
description: unifia sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`unifia serve` komutu, unifia istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `unifia.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `UNIFIA_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `unifia` değeridir; değiştirmek isterseniz `UNIFIA_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `unifia serve` hem de `unifia web` için geçerlidir.
...
`unifia` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
unifia ile programatik etkileşim için unifia sunucusunu kullanın.
:::

Bu mimari, unifia'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`unifia serve` ile bağımsız bir sunucu başlatabilirsiniz. unifia TUI açıksa, `unifia serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum unifia [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
unifia sunucusu aşağıdaki API'leri sunar.
