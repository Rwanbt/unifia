...
_Model Bağlam Protokolü_ veya MCP kullanarak unifia'a harici araçlar ekleyebilirsiniz. unifia hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [unifia Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
unifia, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde unifia şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa unifia, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `unifia mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından unifia, belirteçleri `~/.local/share/opencode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra unifia'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi unifia yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve unifia'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
