...
unifia'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
unifia, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
unifia, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

unifia iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `unifia.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse unifia modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global unifia yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
unifia yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Unifia Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `opencode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `unifia models` komutunu çalıştırın.
:::
