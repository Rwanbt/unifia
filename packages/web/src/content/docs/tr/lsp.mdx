...
description: Unifia, LSP sunucularınızla bütünleşir.
---

Unifia, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

Unifia, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`UNIFIA_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP sunucularını, unifia yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
