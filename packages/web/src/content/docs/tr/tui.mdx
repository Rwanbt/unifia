---
title: TUI
description: Unifia terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unifia, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Unifia'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Unifia TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Unifia'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Unifia'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `unifia.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `UNIFIA_TUI_CONFIG` kullanın.
