---
title: TUI
description: Using the Unifia terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unifia provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Unifia starts the TUI for the current directory.
...
When using the Unifia TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Unifia. Allows you to select from available providers and add their API keys.
...
This is separate from `unifia.json`, which configures server/runtime behavior.
...
Use `UNIFIA_TUI_CONFIG` to load a custom TUI config path.
