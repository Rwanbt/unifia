...
:::caution
模式现在通过 unifia 配置中的 `agent` 选项进行配置。`mode` 选项已废弃。[了解更多](/docs/agents)。
:::

unifia 中的模式允许你为不同的使用场景自定义行为、工具和提示词。

unifia 自带两种内置模式：**build** 和 **plan**。你可以自定义这些模式，也可以通过 unifia 配置创建自己的模式。
...
此路径相对于配置文件所在位置。因此，全局 unifia 配置和项目特定配置均可使用。
