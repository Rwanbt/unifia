...
여기서는 OpenCode용 커뮤니티 제작 CI/CD component인 [nagyv/gitlab-unifia](https://gitlab.com/nagyv/gitlab-unifia)를 사용합니다.
...
- **최소 설정**: CI component가 백그라운드에서 OpenCode를 설정하므로 Unifia config와 초기 prompt만 만들면 됩니다.
...
1. Unifia 인증 JSON을 **Settings** > **CI/CD** > **Variables** 아래의 File 타입 CI 환경 변수로 저장하세요. 반드시 "Masked and hidden"으로 표시하세요.
...
더 많은 입력값과 사용 사례는 이 component의 [docs](https://gitlab.com/explore/catalog/nagyv/gitlab-unifia)에서 확인하세요.
...
댓글에서 `@unifia`를 멘션하면 OpenCode가 GitLab CI 파이프라인 안에서 작업을 실행합니다.
...
:::tip
`@unifia` 대신 다른 trigger phrase를 사용하도록 설정할 수 있습니다.
:::
