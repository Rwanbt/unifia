...
Unifia Zen은 Unifia 팀이 제공하는, 테스트와 검증을 거친 모델 목록입니다.

:::note
Unifia Zen은 현재 beta입니다.
:::

Zen은 OpenCode의 다른 provider와 똑같이 작동합니다. Unifia Zen에 로그인해 API 키를 받으면 됩니다. 이는 **완전히 선택 사항**이며, OpenCode를 사용하기 위해 반드시 사용할 필요는 없습니다.
...
Unifia Zen은 이런 모델에 접근할 수 있게 해주는 AI gateway입니다.

---

## 작동 방식

Unifia Zen은 OpenCode의 다른 provider와 똑같이 작동합니다.

1. **<a href={console}>Unifia Zen</a>**에 로그인하고, 결제 정보를 추가한 뒤, API 키를 복사합니다.
2. TUI에서 `/connect` 명령을 실행하고, Unifia Zen을 선택한 다음 API 키를 붙여넣습니다.
...
Unifia config에서 사용하는 [모델 ID](/docs/config/#models)는 `opencode/<model-id>` 형식입니다. 예를 들어 GPT 5.3 Codex를 사용하려면 config에서 `opencode/gpt-5.3-codex`를 사용하면 됩니다.
