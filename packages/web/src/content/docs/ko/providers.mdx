...
Unifia 설정의 `provider` 섹션을 통해 공급자를 사용자 정의할 수 있습니다.
...
Unifia Zen은 Unifia 팀이 OpenCode와 잘 작동하도록 테스트하고 검증한 모델 목록입니다. [더 알아보기](/docs/zen).

:::tip
처음이라면 Unifia Zen으로 시작하는 것이 좋습니다.
:::

1. TUI에서 `/connect` 명령을 실행하고 `Unifia Zen`을 선택한 뒤, [unifia.ai/auth](https://opencode.ai/zen)로 이동합니다.
...
Unifia Go는 Unifia 팀이 테스트하고 검증하여 OpenCode와 잘 작동하는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있는 저렴한 구독 요금제입니다.

1. TUI에서 `/connect` 명령을 실행하고 `Unifia Go`를 선택한 뒤 [unifia.ai/auth](https://opencode.ai/zen)로 이동하십시오.
...
2. 다음 방법 중 하나를 사용하여 **인증을 구성**합니다:
   ...
   ***
   ...
   프로젝트별 또는 영구 구성을 위해 `unifia.json`을 사용하십시오:
...
OpenCode를 자체 호스팅 GitLab 인스턴스만 사용하도록 제한하려면 `unifia.json` 파일에 다음을 추가하십시오.
...
Helicone의 [Sessions](https://docs.helicone.ai/features/sessions) 기능을 사용하면 관련 LLM 요청을 그룹화할 수 있습니다. [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 플러그인을 사용하여 각 Unifia 대화를 Helicone 세션으로 자동 기록하십시오.
...
이 플러그인은 `Helicone-Session-Id` 및 `Helicone-Session-Name` 헤더를 요청에 주입합니다. Helicone의 세션 페이지에서 각 Unifia 대화가 별도의 세션으로 나열되는 것을 볼 수 있습니다.
...
Unifia Zen은 Unifia 팀에서 제공하는 테스트 및 검증된 모델 목록입니다. [더 알아보기](/docs/zen).

1. **<a href={console}>Unifia Zen</a>**에 로그인하고 **Create API Key**를 클릭합니다.

2. `/connect` 명령을 실행하고 **Unifia Zen**을 검색하십시오.

   ```txt
   /connect
   ```

3. Unifia API 키를 입력하십시오.
...
5. 또한 Unifia 구성을 통해 사용자 정의할 수도 있습니다. 다음은 공급자 순서를 지정하는 예입니다.
...
Unifia 구성을 통해 모델을 사용자 정의할 수도 있습니다. 다음은 공급자 라우팅 순서를 지정하는 예입니다.
...
4. 프로젝트 디렉토리에 `unifia.json` 파일을 만들거나 업데이트하십시오.
...
1. **인증 설정 확인**: `unifia auth list`를 실행하여 공급자의 자격 증명이 구성에 추가되었는지 확인하십시오.

   Amazon Bedrock과 같이 인증을 위해 환경 변수에 의존하는 공급자에는 적용되지 않습니다.

2. 사용자 정의 공급자의 경우, Unifia 구성을 확인하고 다음을 수행하십시오:
   - `/connect` 명령에 사용된 공급자 ID가 Unifia 구성의 ID와 일치하는지 확인하십시오.
