...
3. **Custom config**(`UNIFIA_CONFIG` env var) - custom override
4. **Project config**(프로젝트의 `unifia.json`) - 프로젝트별 설정
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config**(`UNIFIA_CONFIG_CONTENT` env var) - 런타임 override
...
전역 Unifia config는 `~/.config/opencode/opencode.json`에 두세요. provider, model, permissions 같은 사용자 전체 기본 설정은 전역 config로 관리하세요.
...
프로젝트 루트에 `unifia.json`을 추가하세요. 프로젝트 config는 표준 config 파일 중 우선순위가 가장 높아 전역 및 Remote config를 모두 override합니다.
...
Unifia 시작 시 현재 디렉토리에서 config 파일을 찾고, 없으면 가장 가까운 Git 디렉토리까지 상위로 탐색합니다.
...
`UNIFIA_CONFIG` 환경 변수로 custom config 파일 경로를 지정하세요.
...
`UNIFIA_CONFIG_DIR` 환경 변수로 custom config 디렉토리를 지정할 수 있습니다. 이 디렉토리는 표준 `.opencode` 디렉토리와 동일하게 agents, commands, modes, plugins를 검색하며, 동일한 구조를 따라야 합니다.
...
server/runtime config schema는 [**`unifia.ai/config.json`**](https://opencode.ai/config.json)에 정의되어 있습니다.

TUI config는 [**`unifia.ai/tui.json`**](https://opencode.ai/tui.json)을 사용합니다.
...
`UNIFIA_TUI_CONFIG`를 사용하여 사용자 지정 TUI 설정 파일을 가리킬 수 있습니다.

`unifia.json`의 기존 `theme`, `keybinds`, `tui` 키는 더 이상 사용되지 않으며(deprecated) 가능한 경우 자동으로 마이그레이션됩니다.
...
`server` 옵션으로 `unifia serve`와 `unifia web` 명령의 server 설정을 구성할 수 있습니다.
...
- `mdns` - mDNS service discovery를 활성화합니다. 네트워크 내 다른 기기에서 Unifia server를 찾을 수 있습니다.
- `mdnsDomain` - mDNS service의 custom 도메인 이름입니다. 기본값은 `unifia.local`입니다. 같은 네트워크에서 여러 인스턴스를 실행할 때 유용합니다.
...
Unifia config의 `provider`, `model`, `small_model` 옵션으로 사용할 provider와 model을 설정할 수 있습니다.
...
이 설정은 TUI, CLI(`unifia run`), 데스크톱 앱, GitHub Action 등 모든 인터페이스에 적용됩니다.
