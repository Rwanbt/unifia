---
title: IDE
description: VS Code, Cursor 및 기타 IDE용 Unifia 확장 프로그램
---

OpenCode는 VS Code, Cursor, 또는 터미널을 지원하는 모든 IDE와 통합됩니다. 시작하려면 터미널에서 `unifia`를 실행하세요.
...
- **New Session**: `Cmd+Shift+Esc` (Mac) 또는 `Ctrl+Shift+Esc` (Windows/Linux)를 사용해 기존 세션이 열려 있어도 새 Unifia 터미널 세션을 시작하세요. UI의 Unifia 버튼을 클릭해도 됩니다.
...
1. VS Code를 여세요.4
2. 통합 터미널을 여세요.
3. `unifia`를 실행하세요. 확장 프로그램이 자동으로 설치됩니다.
...
Extension Marketplace에서 **Unifia**를 검색한 다음 **Install**을 클릭하세요.
...
- 통합 터미널에서 `unifia`를 실행하고 있는지 확인하세요.
