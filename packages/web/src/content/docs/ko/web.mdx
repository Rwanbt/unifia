---
title: Web
description: 브라우저에서 Unifia 사용하기.
---

opencode는 브라우저에서 웹 응용 프로그램을 실행할 수 있으며 terminal을 필요로하지 않고 동일한 강력한 AI 코딩 경험을 제공합니다.

![unifia Web - 새로운 세션](../../../assets/web/web-homepage-new-session.png)
...
:::caution
`UNIFIA_SERVER_PASSWORD`가 설정되지 않은 경우 서버가 중단되지 않습니다. 이것은 로컬 사용을위한 괜찮지만 네트워크 액세스를 설정해야합니다.
:::

:::tip[Windows 사용자]
최고의 경험을 위해서는 PowerShell보다 `unifia web`를 실행하십시오. 적절한 파일 시스템 액세스 및 terminal 통합을 보장합니다.
:::
...
이 자동으로 `0.0.0.0`에 hostname을 설정하고 `unifia.local`로 서버를 광고합니다.
...
액세스를 보호하려면 `UNIFIA_SERVER_PASSWORD` 환경 변수를 사용하여 암호를 설정하십시오.

```bash
UNIFIA_SERVER_PASSWORD=secret unifia web
```

`unifia`에 대한 사용자의 기본값은 `UNIFIA_SERVER_USERNAME`로 변경할 수 있습니다.

---

## 웹 인터페이스 사용

시작되면 웹 인터페이스는 unifia 세션에 액세스 할 수 있습니다.
...
![unifia Web - Servers 참조](../../../assets/web/web-homepage-see-servers.png)
...
`unifia.json` config 파일에서 서버 설정을 구성할 수 있습니다:
