...
`unifia.json` config 파일에서 agent를 설정합니다.
...
이 경로는 config 파일 위치 기준의 상대 경로입니다. 따라서 전역 Unifia config와 프로젝트별 config 모두에서 동일하게 동작합니다.
...
Unifia config의 model ID는 `provider/model-id` 형식을 사용합니다. 예를 들어 [Unifia Zen](/docs/zen)을 사용한다면 GPT 5.1 Codex에 `opencode/gpt-5.1-codex`를 사용합니다.
...
:::tip
사용 가능한 model 목록은 `unifia models` 명령으로 확인할 수 있습니다.
:::
