...
您可以透過 _Model Context Protocol_（MCP）為 Unifia 新增外部工具。Unifia 同時支援本地和遠端伺服器。
...
您可以在 [Unifia 設定](https://opencode.ai/docs/config/)的 `mcp` 欄位下定義 MCP 伺服器。為每個 MCP 指定一個唯一的名稱，在提示詞中可以透過該名稱來參照對應的 MCP。
...
Unifia 會自動處理遠端 MCP 伺服器的 OAuth 身分驗證。當伺服器需要身分驗證時，Unifia 將：
...
如果伺服器需要身分驗證，Unifia 會在您首次使用時提示您進行認證。您也可以使用 `unifia mcp auth <server-name>` [手動觸發認證流程](#authenticating)。
...
`mcp auth` 指令會開啟瀏覽器進行授權。授權完成後，Unifia 會將 Token 安全地儲存在 `~/.local/share/opencode/mcp-auth.json` 中。
...
您的 MCP 在 Unifia 中作為工具使用，與內建工具並列。因此，您可以像管理其他工具一樣，透過 Unifia 設定來管理它們。
...
這會開啟瀏覽器視窗完成 OAuth 流程，將 Unifia 連結到您的 Sentry 帳號。
