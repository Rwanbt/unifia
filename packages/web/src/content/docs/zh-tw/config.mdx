---
title: 設定
description: 使用 Unifia JSON 設定。
---

您可以使用 JSON 設定檔來設定 Unifia。

---

## 格式

Unifia 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
3. **自訂設定**（`UNIFIA_CONFIG` 環境變數）- 自訂覆寫
4. **專案設定**（專案中的 `unifia.json`）- 專案特定設定
5. **`.opencode` 目錄** - 代理、指令、外掛程式
6. **內嵌設定**（`UNIFIA_CONFIG_CONTENT` 環境變數）- 執行時覆寫
...
將全域 Unifia 設定放在 `~/.config/opencode/opencode.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
在專案根目錄中新增 `unifia.json`。專案設定在標準設定檔中具有最高優先級——它會覆寫全域設定和遠端設定。

:::tip
將專案特定設定放在專案的根目錄中。
:::

當 Unifia 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
使用 `UNIFIA_CONFIG` 環境變數指定自訂設定檔路徑。
...
使用 `UNIFIA_CONFIG_DIR` 環境變數指定自訂設定目錄。該目錄會像標準 `.opencode` 目錄一樣被搜尋代理、指令、模式和外掛程式，並且應遵循相同的結構。
...
設定檔具有在 [**`unifia.ai/config.json`**](https://opencode.ai/config.json) 中定義的 Schema。
...
使用 `UNIFIA_TUI_CONFIG` 指向自訂 TUI 設定檔。

`unifia.json` 中的舊版 `theme`、`keybinds` 和 `tui` 鍵已被棄用，並將在可能的情況下自動遷移。
...
您可以透過 `server` 選項為 `unifia serve` 和 `unifia web` 指令設定伺服器設定。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 Unifia 伺服器。
- `mdnsDomain` - mDNS 服務的自訂網域名稱。預設為 `unifia.local`。適用於在同一網路上執行多個實例的情境。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 Unifia 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，Unifia 會嘗試使用該模型，否則會回退到您的主模型。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，Unifia 將回退到 `"build"` 並發出警告。

此設定適用於所有介面：TUI、CLI（`unifia run`）、桌面應用程式和 GitHub Action。
...
Unifia 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，Unifia **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 Unifia。
...
當您希望限制 Unifia 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
