...
:::caution
模式現在透過 unifia 設定中的 `agent` 選項進行設定。`mode` 選項已廢棄。[了解更多](/docs/agents)。
:::

unifia 中的模式允許您為不同的使用情境自訂行為、工具和提示詞。

unifia 自帶兩種內建模式：**build** 和 **plan**。您可以自訂這些模式，也可以透過 unifia 設定建立自己的模式。
...
此路徑相對於設定檔所在位置。因此，全域 unifia 設定和專案特定設定均可使用。
