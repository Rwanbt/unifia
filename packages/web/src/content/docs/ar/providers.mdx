---
title: المزوّدون
description: استخدام أي موفّر LLM في Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Unifia ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Unifia.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Unifia.
...
Unifia Zen هي قائمة نماذج يوفّرها فريق Unifia وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Unifia. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Unifia Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر unifia، ثم انتقل إلى [unifia.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Unifia واستخدامه اختياري بالكامل.

---

## Unifia Go

Unifia Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Unifia، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Unifia.

1. شغّل الأمر `/connect` في TUI، واختر `Unifia Go`، ثم انتقل إلى [unifia.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Unifia:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل unifia:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `unifia.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Unifia غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل unifia بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات unifia.
...
تخزين الرمز في مساحة تخزين مصادقة unifia.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Unifia نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Unifia
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`unifia.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع unifia:
...
لاستخدام Google Vertex AI مع Unifia:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر unifia، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Unifia تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Unifia مدرجة كجلسة مستقلة.
...
يمكنك ضبط unifia لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط unifia لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط unifia لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Unifia. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Unifia:
...
3. انسخ مفتاح API لاستخدامه في Unifia.
...
6. **مهم**: قبل استخدام نماذج السحابة في Unifia، يجب سحب معلومات النموذج محليا:
...
Unifia Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Unifia. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Unifia Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Unifia Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Unifia.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات unifia.
...
5. يمكنك أيضا تخصيصها عبر إعدادات unifia. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Unifia:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات unifia. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع unifia. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `unifia.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Unifia بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `unifia auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات unifia وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات unifia.
