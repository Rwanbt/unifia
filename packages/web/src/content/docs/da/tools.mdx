...
Her er alle de innebygde verktøyene som er tilgængelige i Unifia.
...
:::note
Dette verktøyet er bare tilgængelig når `UNIFIA_EXPERIMENTAL_LSP_TOOL=true` (eller `UNIFIA_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCode-leverandøren eller når miljøvariabelen `UNIFIA_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
