...
For at fejlfinde problemer med Unifia, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\unifia\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `unifia --log-level DEBUG`.

---

## Lagring

unifia gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\unifia`
...
Unifia Desktop kører en lokal OpenCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Unifia`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\unifia\unifia.jsonc`
...
Unifia kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\unifia\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Unifia kan genopbygge den.

1. Afslut Unifia Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/opencode`
- **Linux**: slet `~/.cache/opencode` (eller kør `rm -rf ~/.cache/opencode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\unifia`

3. Start Unifia Desktop på ny.

---

### Løs problemer med serverforbindelse

Unifia Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `unifia.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `UNIFIA_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `UNIFIA_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Unifia Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Unifia.

---

### Meddelelser vises ikke

Unifia Desktop viser kun systemvarsler når:

- varsler er aktiveret for Unifia i dine OS-indstillinger, og
...
1. Afslut Unifia Desktop.
2. Find og slet disse filer (de findes i Unifia Desktop-appens datamappe):

- `unifia.settings.dat` (skrivebordsstandardserver URL)
- `unifia.global.dat` og `unifia.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Unifia:
...
For at finde ud af hvilke modeller du har adgang til, kør `unifia models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\unifia`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. unifia installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\unifia`

2. Start unifia på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge unifia til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
unifia vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
