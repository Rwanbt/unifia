---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

Unifia bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.
...
Unifia Zen er en liste over modeller leveret af OpenCode-teamet, som er blevet testet og verificeret til at fungere godt med Unifia. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Unifia Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg unifia og gå til [unifia.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i Unifia og er helt valgfrit at bruge.

---

## Unifia Go

Unifia Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenCode-teamet, som er testet og verificeret til at fungere godt med Unifia.

1. Kør kommandoen `/connect` i TUI, vælg `Unifia Go`, og gå til [unifia.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Unifia:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører unifia:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `unifia.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Unifia understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at unifia skal fungere korrekt.
   :::
...
gemme tokenet i unifia auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Unifia bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Unifia
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`unifia.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med Unifia:
...
Sådan bruger du Google Vertex AI med Unifia:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem unifia, kan du altid konfigurere den selv.
...
Du kan konfigurere unifia til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere unifia til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere unifia til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Unifia. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Unifia:
...
3. Kopier API-nøglen til brug i Unifia.
...
6. **Vigtigt**: Før du bruger skymodeller i Unifia, skal du hente modelinformationen lokalt:
...
Unifia Zen er en liste over testede og verificerede modeller leveret af OpenCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Unifia Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Unifia Zen**.

   ```txt
   /connect
   ```

3. Indtast Unifia API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Unifia:
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med unifia. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `unifia.json`-filen i projektkataloget:
...
`limit`-felterne lader Unifia forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `unifia auth list` for at se om legitimationsoplysningerne
