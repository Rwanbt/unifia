...
Unifia giver en interaktiv terminalgrænseflade eller TUI til at arbejde med dine projekter med en LLM.

At køre Unifia starter TUI for den nuværende mappe.
...
Når du bruger Unifia TUI, kan du skrive `/` efterfulgt af et kommandonavn for hurtigt at udføre handlinger. For eksempel:
...
Tilføj en udbyder til Unifia. Lader dig vælge fra tilgængelige udbydere og tilføje deres API-nøgler.
...
Afslut Unifia. _Aliaser_: `/quit`, `/q`
...
Dette er adskilt fra `unifia.json`, som konfigurerer server-/kørselstidsadfærd.
...
Brug `UNIFIA_TUI_CONFIG` til at indlæse en brugerdefineret TUI-konfigurationssti.
