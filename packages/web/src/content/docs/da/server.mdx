...
Kommandoen `unifia serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `unifia.local` |
...
Indstil `UNIFIA_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `unifia`, eller indstil `UNIFIA_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `unifia serve` og `unifia web`.
...
Når du kører `unifia` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med unifia programmatisk.
:::

Denne arkitektur lader unifia understøtte flere klienter og lader dig interagere med unifia programmatisk.

Du kan køre `unifia serve` for at starte en selvstændig server. Hvis du har
unifia TUI kørende, vil `unifia serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Unifia [IDE](/docs/ide) plugins.
