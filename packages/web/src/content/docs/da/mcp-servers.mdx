...
Du kan tilføje eksterne værktøjer til Unifia ved hjælp af _Model Context Protocol_ eller MCP. Unifia understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Unifia Config](https://opencode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Unifia håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Unifia:
...
Hvis serveren kræver godkendelse, vil Unifia bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `unifia mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Unifia opbevare tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Unifia til din Sentry-konto.
