---
title: Plugins
description: Skriv dine egne plugins for at udvide Unifia.
---

Plugins giver dig mulighed for at udvide Unifia ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenCodes standardadfærd.
...
2. Projektkonfiguration (`unifia.json`)
...
Unifia kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En unifia SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide unifia.
...
:::note
Hvis du bruger Unifia desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå unifia i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til unifia:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som unifia kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for unifia sammen med indbyggede værktøjer.
