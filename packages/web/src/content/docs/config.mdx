---
title: Config
description: Using the Unifia JSON config.
---

You can configure Unifia using a JSON config file.

---

## Format

Unifia supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
3. **Custom config** (`UNIFIA_CONFIG` env var) - custom overrides
4. **Project config** (`unifia.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`UNIFIA_CONFIG_CONTENT` env var) - runtime overrides
...
Place your global Unifia config in `~/.config/opencode/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `unifia.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Unifia starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
Specify a custom config file path using the `UNIFIA_CONFIG` environment variable.
...
Specify a custom config directory using the `UNIFIA_CONFIG_DIR`
...
Drop an `unifia.json` or `unifia.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/opencode/` |
| Linux    | `/etc/opencode/`                         |
| Windows  | `%ProgramData%\unifia`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Unifia reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Unifia checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `unifia.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The Unifia config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`unifia.ai/config.json`**](https://opencode.ai/config.json).

TUI config uses [**`unifia.ai/tui.json`**](https://opencode.ai/tui.json).
...
Use `UNIFIA_TUI_CONFIG` to point to a custom TUI config file.

Legacy `theme`, `keybinds`, and `tui` keys in `unifia.json` are deprecated and automatically migrated when possible.
...
You can configure server settings for the `unifia serve` and `unifia web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Unifia server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `unifia.local`. Useful for running multiple instances on the same network.
...
You can configure the providers and models you want to use in your Unifia config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Unifia tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Unifia will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`unifia run`), desktop app, and GitHub Action.
...
Unifia uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Unifia will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, unifia **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Unifia with custom tools, hooks, and integrations.
...
This is useful when you want to restrict Unifia to only use specific providers rather than disabling them one by one.
