---
title: Server
description: Interact with unifia server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `unifia serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an unifia client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `unifia.local` |
...
Set `UNIFIA_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `unifia`, or set `UNIFIA_SERVER_USERNAME` to override it. This applies to both `unifia serve` and `unifia web`.
...
When you run `unifia` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the unifia server to interact with unifia programmatically.
:::

This architecture lets unifia support multiple clients and allows you to interact with unifia programmatically.

You can run `unifia serve` to start a standalone server. If you have the
unifia TUI running, `unifia serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Unifia [IDE](/docs/ide) plugins.
...
The unifia server exposes the following APIs.
