...
:::caution
Modes are now configured through the `agent` option in the unifia config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in unifia allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the unifia config.
...
unifia comes with two built-in modes.
...
Configure modes in your `unifia.json` config file:
...
both the global unifia config and the project specific config.
