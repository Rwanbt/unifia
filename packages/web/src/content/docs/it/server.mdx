---
title: Server
description: Interagisci con il server unifia via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `unifia serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client unifia.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `unifia.local` |
...
Imposta `UNIFIA_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `unifia`, oppure imposta `UNIFIA_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `unifia serve` sia per `unifia web`.
...
Quando esegui `unifia` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server unifia per interagire con unifia in modo programmatico.
:::

Questa architettura permette a unifia di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `unifia serve` per avviare un server standalone. Se la TUI di unifia e gia in esecuzione, `unifia serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Unifia.
...
Il server unifia espone le seguenti API.
