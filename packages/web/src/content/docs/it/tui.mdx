---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Unifia.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Unifia fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Unifia avvia la TUI per la directory corrente.
...
Quando usi la TUI di Unifia, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Unifia. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `unifia.json`, che configura il comportamento di server/runtime.
...
Usa `UNIFIA_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
