...
Puoi aggiungere strumenti esterni a Unifia usando il _Model Context Protocol_, o MCP. Unifia supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Unifia Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Unifia gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Unifia:
...
Se il server richiede autenticazione, Unifia ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `unifia mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Unifia memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Unifia insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Unifia come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Unifia al tuo account Sentry.
