...
In Unifia esistono due tipi di agenti: agenti primari e subagenti.
...
Unifia include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Unifia include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Unifia include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `unifia.json`:
...
Se non specifichi una temperature, Unifia usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Unifia sia per la config specifica del progetto.
...
L'ID modello nella configurazione Unifia usa il formato `provider/model-id`. Per esempio, se stai usando [Unifia Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `unifia models` per vedere la lista dei modelli disponibili.
:::
