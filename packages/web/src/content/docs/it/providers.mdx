---
title: Provider
description: Usa qualsiasi provider LLM in Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

Unifia usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Unifia.
...
Unifia Zen e una lista di modelli forniti dal team Unifia, testati e
verificati per funzionare bene con Unifia. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Unifia Zen.
:::

1. Esegui `/connect` nella TUI, seleziona unifia e vai su [unifia.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in Unifia ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Unifia:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui unifia:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `unifia.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Unifia non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché unifia funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di unifia.
...
salvare il token nello storage di auth di unifia.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Unifia usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Unifia
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `unifia.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `unifia.json`:
...
Per usare il tuo abbonamento GitHub Copilot con unifia:
...
Per usare Google Vertex AI con Unifia:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui unifia.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da unifia, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di Unifia come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Unifia elencata come sessione separata.
...
Puoi configurare unifia per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare unifia per usare modelli locali tramite LM Studio.
...
Puoi configurare unifia per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Unifia. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con Unifia:
...
3. Copia la chiave API da usare in Unifia.
...
6. **Importante**: Prima di usare modelli cloud in Unifia, devi scaricare le informazioni del modello localmente:
...
Unifia Zen è una lista di modelli testati e verificati forniti dal team Unifia. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Unifia Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Unifia Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Unifia.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di unifia.
...
5. Puoi anche personalizzarli tramite la configurazione di unifia. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Unifia:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di unifia. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con unifia. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `unifia.json` nella directory del progetto:
...
I campi `limit` permettono a Unifia di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `unifia auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di unifia e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config unifia.
