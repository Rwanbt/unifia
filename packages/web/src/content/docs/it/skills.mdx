...
Le skill degli agenti permettono a Unifia di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Unifia cerca in queste posizioni:
...
Per i percorsi locali al progetto, Unifia risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
...
Unifia elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `unifia.json`:
...
**Per agenti integrati** (in `unifia.json`):
