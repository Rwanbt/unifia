...
description: Usa la configurazione JSON di Unifia.
---

Puoi configurare Unifia usando un file di configurazione JSON.

---

## Formato

Unifia supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
3. **Config personalizzata** (variabile d'ambiente `UNIFIA_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`unifia.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.opencode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `UNIFIA_CONFIG_CONTENT`) - sovrascritture a runtime
...
Metti la configurazione globale di Unifia in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `unifia.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Unifia si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `UNIFIA_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `UNIFIA_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`unifia.ai/config.json`**](https://opencode.ai/config.json).

La configurazione TUI usa [**`unifia.ai/tui.json`**](https://opencode.ai/tui.json).
...
Usa `UNIFIA_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `unifia.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `unifia serve` e `unifia web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Unifia.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `unifia.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Unifia tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Unifia prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Unifia fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`unifia run`), app desktop e GitHub Action.
...
Unifia scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, unifia **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Unifia con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare Unifia a usare solo alcuni provider invece di disabilitarli uno per uno.
