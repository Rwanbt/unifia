...
Per diagnosticare problemi con Unifia, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\unifia\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `unifia --log-level DEBUG`.

---

## Archiviazione

unifia salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\unifia`
...
Unifia Desktop esegue in background un server locale di Unifia (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Unifia` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\unifia\unifia.jsonc`
...
Unifia puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\unifia\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Unifia possa ricostruirla.

1. Chiudi completamente Unifia Desktop.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\unifia`

3. Riavvia Unifia Desktop.

---

### Risolvi problemi di connessione al server

Unifia Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `unifia.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `UNIFIA_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `UNIFIA_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Unifia Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Unifia.

---

### Notifiche non visualizzate

Unifia Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Unifia nelle impostazioni del sistema operativo, e
...
1. Chiudi Unifia Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Unifia Desktop):

- `unifia.settings.dat` (desktop default server URL)
- `unifia.global.dat` e `unifia.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Unifia:
...
3. Assicurati di avere l'ultima versione con `unifia upgrade`
...
Per capire a quali modelli hai accesso, esegui `unifia models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\unifia`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. unifia installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\unifia`

2. Riavvia unifia per reinstallare i pacchetti provider piu recenti

Questo forzera unifia a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
unifia rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
