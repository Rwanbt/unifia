---
title: Plugin
description: Scrivi plugin per estendere Unifia.
---

I plugin ti permettono di estendere Unifia agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Unifia.
...
2. Config di progetto (`unifia.json`)
...
Unifia esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di unifia per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere unifia.
...
:::note
Se usi l'app desktop di Unifia, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a unifia di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a unifia:
...
L'helper `tool` crea uno strumento personalizzato che unifia può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in unifia insieme agli strumenti integrati.
