...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di unifia. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in unifia ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di unifia.
...
unifia include due modalita integrate.
...
Configura le modalita nel file di configurazione `unifia.json`:
...
Se non viene specificata alcuna temperatura, unifia usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di unifia sia per quella specifica del progetto.
