...
Vous pouvez ajouter des commandes personnalisées via la configuration Unifia ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Unifia [config](/docs/config) :

```json title="unifia.jsonc" {4-12}
{
...
unifia comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
