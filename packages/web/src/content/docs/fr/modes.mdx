...
:::caution
Les modes sont désormais configurés via l'option `agent` dans la configuration unifia.
L’option `mode` est désormais obsolète. [En savoir plus](/docs/agents).
:::

Les modes dans unifia vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.

Il est livré avec deux modes intégrés : **build** et **plan**. Vous pouvez personnaliser ceux-ci ou configurez les vôtres via la configuration unifia.
...
unifia est livré avec deux modes intégrés.
...
Configurez les modes dans votre fichier de configuration `unifia.json` :
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale unifia et la configuration spécifique au projet.
