---
title: Configuration
description: Utilisation de la configuration JSON Unifia.
---

Vous pouvez configurer Unifia à l'aide d'un fichier de configuration JSON.

---

## Format

Unifia prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="unifia.jsonc"
{
...
3. **Configuration personnalisée** (`UNIFIA_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`unifia.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.opencode`** - agents, commandes, plugins
6. **Configuration en ligne** (`UNIFIA_CONFIG_CONTENT` env var) - remplacements d'exécution
...
Placez votre configuration globale Unifia dans `~/.config/opencode/opencode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.
...
Ajoutez `unifia.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Unifia démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `UNIFIA_CONFIG`.

```bash
export UNIFIA_CONFIG=/path/to/my/custom-config.json
unifia run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `UNIFIA_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.opencode`, et devrait suivre la même structure.

```bash
export UNIFIA_CONFIG_DIR=/path/to/my/config-directory
unifia run "Hello world"
```
...
Le fichier de configuration a un schéma défini dans [**`unifia.ai/config.json`**](https://opencode.ai/config.json).

La configuration TUI utilise [**`unifia.ai/tui.json`**](https://opencode.ai/tui.json).
...
Utilisez `UNIFIA_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `unifia.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `unifia serve` et `unifia web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Unifia.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `unifia.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Unifia via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Unifia essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Unifia reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`unifia run`), application de bureau et GitHub Action.
...
Unifia téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, unifia **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Unifia avec des outils, des hooks et des intégrations personnalisés.
...
Ceci est utile lorsque vous souhaitez restreindre Unifia à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
