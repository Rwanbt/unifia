---
title: Plugins
description: Écrivez vos propres plugins pour étendre Unifia.
---

Les plugins vous permettent d'étendre Unifia en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Unifia.
...
2. Configuration du projet (`unifia.json`)
...
Unifia exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK unifia pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre unifia.
...
:::note
Si vous utilisez l'application de bureau Unifia, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez unifia de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à unifia :
...
L'assistant `tool` crée un outil personnalisé que unifia peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour unifia aux côtés des outils intégrés.
