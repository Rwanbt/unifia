...
Unifia Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

:::note
Unifia Go est actuellement en version bêta.
:::

Go fonctionne comme n'importe quel autre fournisseur dans Unifia. Vous vous abonnez à Unifia Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Unifia.
...
Unifia Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

Unifia Go fonctionne comme n'importe quel autre fournisseur dans Unifia.

1. Vous vous connectez à **<a href={console}>Unifia Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `Unifia Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à Unifia Go.
:::
...
Unifia Go inclut les limites suivantes :
...
L'[ID de modèle](/docs/config/#models) dans votre configuration Unifia utilise le format `opencode-go/<model-id>`. Par exemple, pour Kimi K2.5, vous utiliseriez `opencode-go/kimi-k2.5` dans votre configuration.
...
Nous avons créé Unifia Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec Unifia également.
