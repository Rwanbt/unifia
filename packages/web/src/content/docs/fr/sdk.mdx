---
title: SDK
description: Client JS de type sécurisé pour le serveur unifia.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK unifia JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler unifia par programme.
...
Créez une instance de unifia :

```javascript
import { createUnifia } from "@unifia/sdk"

const { client } = await createUnifia()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `unifia.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${unifia.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de unifia, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@unifia/sdk"
```
