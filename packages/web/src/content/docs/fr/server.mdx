---
title: Serveur
description: Interagissez avec le serveur unifia via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `unifia serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client unifia peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `unifia.local` |
...
Définissez `UNIFIA_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `unifia`, ou définissez `UNIFIA_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `unifia serve` et à `unifia web`.
...
Lorsque vous exécutez `unifia`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur unifia pour interagir avec unifia par programmation.
:::

Cette architecture permet à unifia de prendre en charge plusieurs clients et vous permet d'interagir avec unifia par programme.

Vous pouvez exécuter `unifia serve` pour démarrer un serveur autonome. Si vous avez le
unifia TUI en cours d'exécution, `unifia serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Unifia [IDE](/docs/ide).
...
Le serveur unifia expose les API suivantes.
