...
Les compétences d'agent permettent à Unifia de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Unifia recherche ces emplacements :
...
Pour les chemins locaux du projet, Unifia remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
...
Unifia répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `unifia.json` :
...
**Pour les agents intégrés** (dans `unifia.json`) :
