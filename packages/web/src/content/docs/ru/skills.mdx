...
Навыки агента позволяют unifia обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
unifia выполняет поиск в следующих местах:
...
Для локальных путей проекта unifia переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
...
unifia перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `unifia.json`:
...
**Для встроенных агентов** (в формате `unifia.json`):
