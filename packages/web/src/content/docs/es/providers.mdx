---
title: Proveedores
description: Usando cualquier proveedor LLM en Unifia.
---

import config from "../../../../config.mjs"
export const console = config.console

Unifia usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Unifia.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Unifia
configuración.
...
Unifia Zen es una lista de modelos proporcionados por el equipo Unifia que han sido
probado y verificado para funcionar bien con Unifia. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Unifia Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione unifia y diríjase a [unifia.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en Unifia y su uso es completamente opcional.

---

## Unifia Go

Unifia Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Unifia que han sido
probado y verificado para funcionar bien con Unifia.

1. Ejecute el comando `/connect` en TUI, seleccione `Unifia Go` y diríjase a [unifia.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con Unifia:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta unifia:
   ...
   Para una configuración persistente o específica del proyecto, utilice `unifia.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Unifia no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que unifia funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración unifia.
...
para almacenar el token en el almacenamiento de autenticación unifia.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Unifia utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Unifia
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `unifia.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `unifia.json`:
...
Para utilizar su suscripción GitHub Copilot con unifia:
...
Para utilizar Google Vertex AI con Unifia:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta unifia.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de unifia, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Unifia como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Unifia enumerada como una sesión separada.
...
Puede configurar unifia para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar unifia para usar modelos locales a través de LM Studio.
...
Puedes configurar unifia para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Unifia. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Unifia:
...
3. Copie la clave API para usarla en Unifia.
...
6. **Importante**: Antes de usar modelos en la nube en Unifia, debe obtener la información del modelo localmente:
...
Unifia Zen es una lista de modelos probados y verificados proporcionada por el equipo Unifia. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Unifia Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Unifia Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Unifia API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración unifia.
...
5. También puedes personalizarlos a través de tu configuración unifia. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Unifia:
...
También puede personalizar modelos a través de su configuración unifia. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con unifia. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `unifia.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Unifia comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `unifia auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración unifia y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración unifia.
