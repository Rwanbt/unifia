...
Las herramientas permiten que LLM realice acciones en su código base. Unifia viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Unifia.
...
:::note
Esta herramienta solo está disponible cuando `UNIFIA_EXPERIMENTAL_LSP_TOOL=true` (o `UNIFIA_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Unifia o cuando la variable de entorno `UNIFIA_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Unifia:

```bash
UNIFIA_ENABLE_EXA=1 unifia
```
