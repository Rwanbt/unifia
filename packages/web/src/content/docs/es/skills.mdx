...
Las habilidades del agente permiten a Unifia descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Unifia busca estas ubicaciones:
...
Para las rutas locales del proyecto, Unifia sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
...
Unifia enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `unifia.json`:
...
**Para agentes integrados** (en `unifia.json`):
