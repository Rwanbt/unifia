---
title: Servidor
description: Interactuar con el servidor unifia a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `unifia serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente unifia puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `unifia.local` |
...
Configure `UNIFIA_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `unifia`, o configure `UNIFIA_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `unifia serve` como a `unifia web`.
...
Cuando ejecuta `unifia`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor unifia para interactuar con unifia mediante programación.
:::

Esta arquitectura permite que unifia admita múltiples clientes y le permite interactuar con unifia mediante programación.

Puede ejecutar `unifia serve` para iniciar un servidor independiente. Si tienes el
unifia TUI ejecutándose, `unifia serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Unifia [IDE](/docs/ide).
...
El servidor unifia expone las siguientes API.
