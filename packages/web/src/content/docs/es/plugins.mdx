---
title: Complementos
description: Escriba sus propios complementos para extender Unifia.
---

Los complementos le permiten extender Unifia al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Unifia.
...
2. Configuración del proyecto (`unifia.json`)
...
Unifia ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK unifia para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar unifia.
  ...
  return {
    event: async ({ event }) => {
      ...
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "unifia"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Unifia, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que unifia lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a unifia:

```ts title=".opencode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@unifia/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que unifia puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para unifia junto con las herramientas integradas.
