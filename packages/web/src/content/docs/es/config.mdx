---
title: Configuración
description: Usando la configuración Unifia JSON.
---

Puede configurar Unifia usando un archivo de configuración JSON.

---

## Formato

Unifia admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Por ejemplo, si su configuración global establece `theme: "unifia"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
3. **Configuración personalizada** (`UNIFIA_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`unifia.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.opencode`** - agentes, comandos, complementos
6. **Configuración en línea** (`UNIFIA_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
Coloque su configuración global Unifia en `~/.config/opencode/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `unifia.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia Unifia, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `UNIFIA_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `UNIFIA_CONFIG_DIR`
...
El archivo de configuración tiene un esquema definido en [**`unifia.ai/config.json`**](https://opencode.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `unifia serve` y `unifia web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Unifia.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `unifia.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Unifia a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Unifia intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Unifia a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Unifia recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`unifia run`), aplicación de escritorio y GitHub Acción.
...
Unifia descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, unifia **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían Unifia con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir Unifia para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
