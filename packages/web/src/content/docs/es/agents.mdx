...
Hay dos tipos de agentes en Unifia; agentes primarios y subagentes.
...
Unifia viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Unifia viene con dos subagentes integrados, **General** y **Explore**. Veremos esto a continuación.

---

## Integrados

Unifia viene con dos agentes primarios integrados y dos subagentes integrados.
...
Configure agentes en su archivo de configuración `unifia.json`:
...
Si no se especifica ninguna temperatura, Unifia utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Unifia como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Unifia usa el formato `provider/model-id`. Por ejemplo, si usa [Unifia Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `unifia models` para ver una lista de los modelos disponibles.
:::
