...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "unifia.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "unifia.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "unifia.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "unifia.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["unifia.json", "unifia.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "unifia.json"),
        path.join(tmp.path, "unifia.jsonc"),
        path.join(project, "unifia.json"),
        path.join(project, "unifia.jsonc"),
      ])
    })
  })
