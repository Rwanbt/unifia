...
- Author package entrypoint is `@unifia/plugin/tui`.
...
- Import types from `@unifia/plugin/tui`.
- `@unifia/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@unifia/plugin/tui"
...
- The value is a semver range checked against the running Unifia version.
...
- `unifia plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `unifia plug <module>`.
...
When `plugin` entries exist in a writable `.opencode` dir or `UNIFIA_CONFIG_DIR`, Unifia installs `@unifia/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@unifia/plugin/tui`.
...
- `--pure` / `UNIFIA_PURE` skips external TUI plugins only.
