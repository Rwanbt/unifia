...
export function integrationValidationCommands(plan: IntegrationPlan): IntegrationValidationCommand[] {
  ...
  if ([...changedPaths].some((changedPath) => changedPath.startsWith("packages/opencode/"))) {
    commands.push({ label: "packages/opencode typecheck", cwd: "packages/opencode", argv: ["bun", "run", "typecheck"] })
  }
