...
**Current worktree:** local `voice` is at `a706dcc0d4` and is four commits ahead of `origin/voice` (`89945700f0de04206e67844f011c614e4b41911d`). Push is blocked: the configured proxy is unavailable, GitHub CLI credentials are invalid, and the GitHub connector denied Git blob creation with HTTP 403. Do not force-push or recreate commits through another API. The latest VoiceCore + TypeScript contract slice passes strict local checks; remote CI is pending. VoiceCore production event producers have not migrated. Pre-existing build/cache artifacts remain unstaged.
...
1. Resolve GitHub write authentication/proxy availability, then push the four local commits and inspect exact-SHA CI results; never force-push or recreate commits through an API.
