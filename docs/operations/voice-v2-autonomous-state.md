...
| G2 — Shared VoiceCore | In progress | Portable `packages/voice-core` defines typed error/event contracts, sequence assignment, idempotency-key deduplication, monotonic timestamp checks bounded to JavaScript safe integers, generation fencing, turn tokens, playback-only cancellation, reconnect, and snapshot recovery. Rust tests cover duplicate retries, ordering, invalid-event atomicity, safe-integer boundaries, stale generations, cancellation races, reconnect, and recovery; the Tauri runtime links the crate. At `65ff1b6cbcec3650bb26d751e01f5d41e0de0205`, the Python error/readiness producer starts at sequence 0 like VoiceCore and rejects negative, fractional, boolean, or unsafe-integer sequences; the full Voice Host suite passes. Production producers still do not use VoiceCore as their ordering/state authority. Complete lifecycle state machine, durable persistence, adapters, generation parity, and cross-runtime fixture parity remain open.
...
1. Restore valid GitHub write credentials, verify the live `voice` ref, push the nine local commits normally, then inspect exact-SHA CI results; never force-push or recreate commits through an API.
