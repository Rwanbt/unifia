...
**IN PROGRESS — NOT GO PROD.** Live startup checks an explicit model against the configured connected-provider catalog before `voice_ready`, without creating a session or generating a turn; this does not prove inference or provider reachability. Parakeet has immutable archive and per-file SHA-256 pins and a shared atomic installer. The Android local Live final-transcript path durably reserves the same canonical message ID used by the SDK before prompting and sequences semantic events through VoiceCore; snapshots retain replay protection and clock state, not event payloads. Python and desktop producers still bypass VoiceCore; streaming AgentBridge, lifecycle/recovery parity, safe history-capacity rotation and cross-runtime fixtures remain open. No production qualification is inferred from unit tests or scaffolds.
...
| G6 — AgentBridge streaming | In progress | Android local Live now uses the real SDK `promptAsync` endpoint and its event feed; first correlated text deltas reach the Live controller without waiting for response completion. The controller still waits for the completed answer before TTS, so SpeechSegmenter streaming is not qualified.
