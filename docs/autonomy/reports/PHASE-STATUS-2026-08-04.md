...
| 12 | Artifact Studio | `PARTIEL` | **Lignage de versions livré** (`47b1b84`, `ArtifactStore` 27/27) : l'id nomme une lignée indépendante du contenu, chaque révision a son manifeste, `history`/`latest`, révisions antérieures préservées, ré-écriture idempotente, et deux lignées au contenu identique coexistent (le bug de collision est corrigé). Export vers outbox avec politique de métadonnées `strip` par défaut. **Restent `NON FAIT`** : diff sémantique DOCX/PPTX/XLSX, preview sandboxée, et le stripping des métadonnées **de format** (EXIF, docProps OOXML, Info PDF) — la politique actuelle ne couvre que l'enregistrement de métadonnées Unifia, limite écrite dans le code. |
