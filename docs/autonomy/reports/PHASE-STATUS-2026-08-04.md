...
| 5 | Extraction OpenWork (serveur, orchestrateur) | `PASS local` (serveur) / `PARTIEL` | `WorkbenchServer` 72/72 + **`WorkbenchBootstrap` 39/39 sur HTTP réel** (`5590c9d`) : le serveur est désormais un processus, avec écoute loopback, audit JSONL durable et arrêt propre. Le critère « le serveur fonctionne headless » est atteint localement. **Orchestrateur et MultiWorkspaceRouter livrés** (`@unifia/workbench-orchestrator` 28/28) : le critère de sortie « multi-workspace fonctionne sans relancer inutilement le cœur » est mesuré — 25 changements de workspace, **1 seul runtime construit**. L'isolation passe par le scoping, pas par la multiplication de runtimes ; le routeur refiltre les sessions au cas où le runtime ignorerait le scope. |
