...
> Statut : **PUSHED**
> Phase : **M1-11-V1-MIGRATION-DELIVERED-2026-09-03 + ADR-000-RECONCILED-2026-09-03** (M2+M3 = 538/0 tests V2 ; PostM3-R1 = 18 cartes GREEN ; PostM3-R2 = 23 cartes GREEN ; **M0 contract feasibility 36/36 + minimal reference substrate 15/15 = 51/51 PASS — RECLASSIFIED 2026-09-03 (review v1.1) comme feasibility du contrat, PAS substrate selection proof** ; **M1-09 in-memory 14/14 + M1-10 file-backed 7/7 = 21/21 PASS, transition matrix per ADR-022 §4, JSON snapshot atomic persistence, fail-closed on corruption** ; **M1-11 V1→V2 history migration 6/6 PASS, V1MigratingAuthority wraps any DurableHistoryAuthority, refuses block warnings (e.g. shell), registers run "running" then atomic transition to V1 final status** ; **M2-07/08/09 control.while/child node families + parseurs + 25/25 contract tests GREEN** ; **3 gates RED→GREEN : unit_automate_surface (C-PRE1-01 livré, 17 tests) + unit_workflow_runtime (27 tests) + v2_contracts_regression (809/809 tests)** ; **ADR-031 re-préparé DECIDED post-M0** ; **ADR-000 live fichier = résumé non-normatif pointant vers ADR-000-DURABLE-EXECUTION-SUBSTRATE-M0-FROZEN.md (2184 lignes, FROZEN, source canonique)**). **32/32 ADR V2.3.1 ecrits** (30 DECIDED, 1 INFORMATIONAL, 1 CHANGES_REQUIRED). **851/0 tests V2 (632 contracts + 27 workflow-runtime + 177 m0-contract + 15 m0-harness) + 32 migration tool, 0 fail**. **Cert runner : 15 GREEN, 3 RED, 1 NA, 7 OTHER (NOT_BUILT/NOT_READY/NEEDS_RE_RUN), 5 BLOCKER (sur 25 gates)**. **123 commits sur origin/agent/automate-v2-baseline-20260901** le 2026-09-03. M0 contract feasibility PASS. M1-09 + M1-10 + M1-11 DECIDED. M2-07/08/09 contracts DELIVERED (runtime blocked ADR-000). M0 comparatif Native ↔ DBOS Go = **NEXT WORKSTREAM** (per pack gelé). ADR-000 statut réel = `NOT_RATIFIED` (per pack gelé).)
> Date : 2026-09-03
...
| M3-09 timeouts (contrat) | **DONE** (GREEN, 15/15 PASS) | contracts/src/timeout.ts + timeout.test.ts (1bc49935e5) |
| M3-09 timeouts (impl runtime) | **BLOCKED** (RED) | bloqué ADR-000 |
...
| M0 proof contract half (M0-1..M0-10) | **DONE** (36/36 PASS = M0_CONTRACT_FEASIBILITY) | packages/automate-m0-contract/test/m0-proof.test.ts (d12bdad3dc) |
| M0 proof runtime half (M0-1..M0-10) | **DONE** (15/15 PASS = MINIMAL_REFERENCE_SUBSTRATE) | packages/automate-m0-harness/test/m0-runtime.test.ts (28e3058110) |
| M0 proof gate (combined) | **RECLASSIFIED 2026-09-03 (review v1.1)** : 51/51 = feasibility du contrat + reference substrate, **PAS** substrate selection proof | per ADR-000 §7 (live ADR désormais résumé non-normatif pointant vers pack gelé) |
