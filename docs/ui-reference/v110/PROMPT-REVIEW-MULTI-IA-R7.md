...
# Prompt de review indépendante — PLAN R7 (issue #116)

Copier ce prompt avec les trois fichiers suivants :

1. `PLAN-PIXEL-PERFECT-PORT-2026-09-17-R7.md` ;
2. les synthèses R3 à R6 ;
3. la maquette ou un accès lecture au dépôt.
...
Tu reviews en lecture seule le PLAN R7 de port visuel v110 d'Unifia. Le plan existe parce
...
branch    new-ui
plan      docs/ui-reference/v110/PLAN-PIXEL-PERFECT-PORT-2026-09-17-R7.md
...
7. Ne recopie pas les findings R3–R6 sans vérifier que R7 ne les a pas déjà fermés.
8. Distingue fait exécuté, observation de code et hypothèse.
9. Après R7, un finding documentaire sans preuve nouvelle de confiance ≥0,7 ne rouvre pas le plan.

## Rôles

### ROLE=A — factualité et contrat

Vérifie : PF0, modes G0 bootstrap/pilot/tool-change/full, commits et artifacts antérieurs,
ordre S2→QF0, contenu exhaustif des locks, manifests mutables, StaticCase/MotionCase,
census DOM+AST+trace, cardinalités, clés, ledger, profils et scan `data-parity`.
...
- retirer une ancre dynamique, un état dû ou tous les résultats `reduce` ;
- modifier checker et lock dans la même PR ;
- laisser un path verrouillé dirty dans index/worktree ;
- promouvoir avec `notRun > 0` ou exécuter QF0 avant S2 ;
...
Vérifie : image/Bun/fontSet runtime, contextes, mutations harness/interaction, motion
none/finite/continuous, phase/cycle et anti-alias statique, A/A provenant de P0a, résidu,
masques, census SVG/pseudo/focusable/handler, responsive, DPR2, thèmes et locales.
...
- créer une différence uniquement au hover ou injecter un focusable invisible côté app ;
- déplacer parent et enfant en sens opposés ;
- retirer la preuve de référence.
- muter le DOM de référence ou le lockfile ;
- désactiver une animation attendue ou laisser une animation continue sans phase ;

### ROLE=D — exécution autonome et DX

Vérifie en simulation froide PF0, P0a, P0b, P1, S0, S1, S2, QF0, S3, S7, S8 et S15. Contrôle les chemins,
...
- vérifier que les trois ancres passent le profil pilote final dans P1, quatrième PR de la séquence ;
- vérifier que les outputs P1 sont artifacts CI du même run, pas une PR séparée ;
- vérifier `MANUAL_MERGE_ONLY` puisque les deux branches sont actuellement non protégées ;
...
1. PF0/P0a/P0b rendent-ils P1 atteignable en quatrième PR sans auto-signature ?
...
5. Un état, motion profile, DPR, locale ou nœud app surnuméraire peut-il échapper au calcul ?
...
Le plan R7 est actuellement **PROPOSÉ**. La review ne doit ni commencer l'implémentation ni
