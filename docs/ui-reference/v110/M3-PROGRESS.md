...
| fix | **#92** — seed d'un registry model-intelligence vide au boot e2e (`empty-registry.ts` + 4 tests unitaires) ; le contournement 503 de `gate.ts` est retiré, contrat zéro-console-error rétabli ; route vérifiée 200 `{"items":[],"total":0}` en process | `5f63a1418c` | LIVREE |
...
| #92 — seed registry e2e | 4/4 tests + route 200 vérifiée | `empty-registry.test.ts` ; `GET /model-intelligence/models` → 200 `{"items":[],"total":0}` (était 503) ; suite model-intelligence 424 pass |
