...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/unifia")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/unifia")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with unifia...`)
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/unifia`.",
  ...
  try {
    await $`unifia run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  unifia failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with unifia")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with unifia...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/unifia`.",
    "Run `./script/build.ts --single` in `packages/unifia`.",
