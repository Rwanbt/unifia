...
サーバーモードはオプトイン方式のみです。有効化時には `UNIFIA_SERVER_PASSWORD` を設定して HTTP Basic Auth を強制してください。これがないと、サーバーは認証なしで動作します（警告あり）。サーバーを保護するのはエンドユーザーの責任です — それが提供するいかなる機能も脆弱性ではありません。
